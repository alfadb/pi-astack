...
export const D3_V2_R4_REVISION = "R4" as const;
...
export const D3_V2_R4_PRODUCTION_SESSION_ID = "019f77f6-899b-7fa0-ad5f-e1d8bb1ceadc" as const;
export const D3_V2_R4_PRODUCTION_SESSION_ROOT = "/home/worker/.pi/agent/sessions" as const;
export const D3_V2_R4_PRODUCTION_SESSION_PATH = "/home/worker/.pi/agent/sessions/--home-worker-.pi--/2026-07-19T01-21-13-627Z_019f77f6-899b-7fa0-ad5f-e1d8bb1ceadc.jsonl" as const;
...
export const D3_V2_R4_PRODUCTION_QUARANTINE_TARGET = `/home/worker/.pi/agent/sessions/--home-worker-.pi--/.adr0040-r4-quarantine-${D3_V2_R4_PRODUCTION_SESSION_ID}.jsonl` as const;
...
export function buildD3V2R4OperatorManifest(repoRootInput: string): D3V2R4OperatorManifest {
  ...
  const graph = buildTypescriptStaticDependencyGraph({
    repoRoot,
    roots: [D3_V2_R4_MODULE, "extensions/abrain/rule-injector/proposition-lifecycle-freshness-d3-v2-session-start-control.ts"],
    explicitFiles: [
      "extensions/abrain/rule-injector/index.ts", "pi-astack-settings.schema.json", "package.json", "package-lock.json",
...
export function buildD3V2R4FrozenBinding(args: {
  ...
  sessionsRoot: string;
  sessionPath: string;
  frozenPrefix?: Readonly<{ path: string; dev: number; ino: number; prefix_bytes: number; prefix_sha256: string }>;
  settingsPath: string;
  controlRoot: string;
  ...
  assertSafeSessionIdComponent(args.sessionId);
  const session = args.frozenPrefix ?? captureTrustedSessionPrefixBinding({ sessionsRoot: args.sessionsRoot, sessionPath: args.sessionPath, expectedSessionId: args.sessionId });
  assertSessionBindingCurrent(session, args.sessionsRoot, args.sessionId);
  const settings = captureD3V2R4SettingsPrestate(args.settingsPath);
  assertNoSelectorConflict(settings.parsed, args.sessionId, true);
  ...
  const desired = buildD3V2R4DesiredSettings({ sessionId: args.sessionId, d3: args.d3, adapterManifestHash: args.adapterManifestHash, r4Binding });
  const postRaw = renderD3V2R4SettingsPost(settings.parsed, desired);
  return deepFreeze({
    schema_version: "adr0040-d3-v2-session-start-r4-frozen-execution-binding/v1",
    session_id: args.sessionId,
    sessions_root: path.resolve(args.sessionsRoot),
    ...
    rollback_target: path.resolve(args.controlRoot),
...
export function executeD3V2R4BindOperator(args: {
  ...
  assertSessionBindingCurrent(args.frozen.session_file, args.frozen.sessions_root, args.frozen.session_id);
...
function executeFresh(args: {
  ...
  assertNoSelectorConflict(settingsPre.parsed, frozen.session_id, true);
  ...
  const coordinate = verifyFreshLatestStandaloneUserAuthorization({
    sessionsRoot: frozen.sessions_root,
    sessionPath: frozen.session_file.path,
    expectedSessionId: frozen.session_id,
  ...
  const intentBase = {
    schema_version: D3_V2_R4_INTENT_SCHEMA,
  ...
  try {
    // Authorization and all preconditions are verified before the first create.
    ensureR4ControlDirectories(frozen.control_root);
    assertCreateTargetsAbsent(paths);
    publishCreateOnlyRetained(paths.intent, `${canonicalizeJcs(intent)}\n`, "R4 create-only intent");
    publishCreateOnlyRetained(paths.activation, `${canonicalizeJcs(activation)}\n`, "R4 create-only activation");
    const postRaw = renderD3V2R4SettingsPost(settingsPre.parsed, frozen.desired_settings);
    const cas = applySettingsCas({
      settingsPath: frozen.settings_path,
      parentFd: lock.fd,
      expectedPre: frozen.settings_pre,
      postRaw,
      desired: frozen.desired_settings,
      beforeCas: args.testHooks?.beforeSettingsCas,
    });
    ...
    publishCreateOnlyRetained(paths.receipt, `${canonicalizeJcs(receipt)}\n`, "R4 create-only commit receipt");
...
function executeContinue(args: {
  ...
  const frozen = args.frozen;
  const found = loadSoleOperation(frozen.control_root);
  ...
  assertTupleMatchesFrozen(tuple, frozen, requireSourceCommit(args.target, frozen));
  const paths = operationPaths(frozen.control_root, operationId, frozen.operator_audit_path);
  const activation = loadActivation(paths.activation, operationId, String(intent.intent_hash));
  ...
  try {
    ...
    const currentHash = settings.identity.raw_sha256;
    const receiptExists = fileExistsNoFollow(paths.receipt);
    const isPre = currentHash === frozen.settings_pre.raw_sha256 && sameFileIdentitySansRaw(settings.identity, frozen.settings_pre);
    const isPost = currentHash === frozen.settings_post_raw_sha256 && settingsSemanticallyExact(settings.raw, frozen.desired_settings);
    if (receiptExists) {
      const receipt = loadReceipt(paths.receipt, operationId, String(intent.intent_hash), activation.activation_object_hash);
      assertReceiptMatchesTuple(receipt, tuple, intent, activation, paths);
      if (!isPost) fail("R4_RECOVERY_INVALID", "receipt present while settings is not exact post; halt");
      const terminal = verifyTerminalState(frozen, operationId, intent, activation, receipt);
      return deepFreeze({ status: "terminal_verified", operation_id: operationId, receipt_hash: receipt.receipt_hash, rewritten: false, runtime_injection_authorized: false, runtime_audit_required_later: true, terminal });
    }
    if (!isPre && !isPost) fail("R4_RECOVERY_INVALID", "settings is neither exact pre nor exact post; halt");
    ...
    publishCreateOnlyRetained(paths.receipt, `${canonicalizeJcs(receipt)}\n`, "R4 continue create-only commit receipt");
    const terminal = verifyTerminalState(frozen, operationId, intent, activation, receipt);
    ...
    return deepFreeze({ status: "bound", operation_id: operationId, receipt_hash: receipt.receipt_hash, settings_cas: casStatus, runtime_injection_authorized: false, runtime_audit_required_later: true, operator_audit: audit, terminal });
  } finally { lock.close(); }
}
...
export function evaluateD3V2R4RuntimeGate(args: {
  ...
  try {
    ...
    const target = asRecord(tuple.target_session, "R4 runtime target session");
    if (target.session_id !== sessionId || path.resolve(String(target.path)) !== path.resolve(sessionPath)) fail("R4_RUNTIME_SESSION", "runtime session path/id differs from authorization tuple");
    assertSessionBindingCurrent({ path: String(target.path), dev: Number(target.dev), ino: Number(target.ino), prefix_bytes: Number(target.prefix_bytes), prefix_sha256: String(target.prefix_sha256) }, path.dirname(path.dirname(path.resolve(sessionPath))), String(sessionId), false);
    ...
    assertReceiptMatchesTuple(receipt, tuple, intent, activation, paths);
    const completion = validateTrustedSessionUserCoordinate(asRecord(receipt.completion_authorization, "R4 runtime completion authorization").coordinate);
    assertSessionBindingCurrent({
      path: completion.session_jsonl_path,
      dev: completion.session_dev,
      ino: completion.session_ino,
      prefix_bytes: completion.transcript_prefix_bytes,
      prefix_sha256: completion.transcript_prefix_sha256,
    }, path.dirname(path.dirname(path.resolve(sessionPath))), String(sessionId), false);
    ...
    if (receipt.session_file && canonicalizeJcs(receipt.session_file) !== canonicalizeJcs(target)) fail("R4_RUNTIME_SESSION", "receipt session binding differs");
...
function buildAuthorizationTuple(frozen: D3V2R4FrozenBinding, coordinate: TrustedSessionUserCoordinate, sourceCommit: string): Readonly<Record<string, unknown>> {
  const tuple = {
    ...
    target_session: deepFreeze({ session_id: frozen.session_id, ...frozen.session_file }),
...
function buildActivation(frozen: D3V2R4FrozenBinding, coordinate: TrustedSessionUserCoordinate, operationId: string, intentHash: string): D3V2BoundActivationObject {
  ...
  const value = buildD3V2SessionStartActivationObject({
    ...
    sessionFile: frozen.session_file,
...
function buildReceipt(args: {
  ...
  const base = {
    ...
    session_file: deepFreeze({ session_id: args.frozen.session_id, ...args.frozen.session_file }),
...
function verifyTerminalState(frozen: D3V2R4FrozenBinding, operationId: string, intentInput: Record<string, unknown>, activation: D3V2BoundActivationObject, receiptInput: Record<string, unknown>): Readonly<Record<string, unknown>> {
  ...
  assertSessionBindingCurrent(frozen.session_file, frozen.sessions_root, frozen.session_id);
...
function validateAuthorizationTuple(tuple: Record<string, unknown>): void {
  ...
  if (typeof tuple.source_commit !== "string" || !/^[0-9a-f]{40}$/.test(tuple.source_commit)) fail("R4_AUTHORIZATION_TUPLE", "authorization tuple source_commit must be a clean Git commit");
  const target = asRecord(tuple.target_session, "authorization tuple target_session");
  exactKeys(target, ["session_id", "path", "dev", "ino", "prefix_bytes", "prefix_sha256"], "authorization tuple target_session");
  assertSafeSessionIdComponent(target.session_id);
  const targetPath = requireAbsolute(String(target.path), "authorization tuple session path");
  assertHash(target.prefix_sha256, "target prefix_sha256");
  for (const field of ["dev", "ino", "prefix_bytes"] as const) {
    if (!Number.isSafeInteger(target[field]) || Number(target[field]) < 1) fail("R4_AUTHORIZATION_TUPLE", `target_session.${field} is invalid`);
  }
  if (coordinate.session_id !== target.session_id || coordinate.session_jsonl_path !== targetPath
    || coordinate.session_dev !== target.dev || coordinate.session_ino !== target.ino
    || coordinate.transcript_prefix_bytes < Number(target.prefix_bytes)
    || (coordinate.transcript_prefix_bytes === target.prefix_bytes && coordinate.transcript_prefix_sha256 !== target.prefix_sha256)) {
    fail("R4_AUTHORIZATION_TUPLE", "authorization coordinate does not bind the exact target session identity/prefix");
  }
  ...
  if (coordinate.text_utf8_bytes !== contract.required_phrase_utf8_bytes || coordinate.text_sha256 !== contract.required_phrase_sha256) {
    fail("R4_AUTHORIZATION_TUPLE", "initial authorization coordinate text does not match its exact contract");
  }
}
...
function assertActivationMatchesTuple(activation: D3V2BoundActivationObject, tuple: Record<string, unknown>, intent: Record<string, unknown>, paths: ReturnType<typeof operationPaths>): void {
  const target = asRecord(tuple.target_session, "activation target session");
  ...
  if (activation.schema_version !== D3_V2_SESSION_START_R4_ACTIVATION_OBJECT_SCHEMA
    ...
    || activation.session_id !== target.session_id
    ...
    || canonicalizeJcs(activation.session_file) !== canonicalizeJcs({ path: target.path, dev: target.dev, ino: target.ino, prefix_bytes: target.prefix_bytes, prefix_sha256: target.prefix_sha256 })
...
function assertReceiptMatchesTuple(receipt: Record<string, unknown>, tuple: Record<string, unknown>, intent: Record<string, unknown>, activation: D3V2BoundActivationObject, paths: ReturnType<typeof operationPaths>): void {
  const target = asRecord(tuple.target_session, "receipt target session");
  ...
  if (receipt.operation_id !== intent.operation_id || receipt.intent_hash !== intent.intent_hash
    ...
    || canonicalizeJcs(receipt.session_file) !== canonicalizeJcs({ session_id: target.session_id, path: target.path, dev: target.dev, ino: target.ino, prefix_bytes: target.prefix_bytes, prefix_sha256: target.prefix_sha256 })
  ...
  assertCompletionAuthorizationMatchesTuple(receipt, initial, target);
}
...
function assertCompletionAuthorizationMatchesTuple(
  ...
  if (coordinate.session_id !== target.session_id || coordinate.session_jsonl_path !== target.path
    || coordinate.session_dev !== target.dev || coordinate.session_ino !== target.ino
    ...
    fail("R4_RECEIPT_INVALID", "fresh continue receipt coordinate does not bind a later exact target-session authorization");
  }
}

function validateReceipt(value: unknown): Record<string, unknown> {
  const receipt = asRecord(value, "R4 receipt");
  exactKeys(receipt, ["schema_version", "operation_id", "intent_hash", "activation_object_hash", "settings_pre_raw_sha256", "settings_post_raw_sha256", "settings_post_identity", "session_file", "d3_identities", "adapter_manifest_hash", "operator_manifest_hash", "predecessor_dossier", "execution_dossier", "source_closure_hash", "initial_authorization_coordinate_hash", "completion_authorization", "control_paths", "durable_object_not_message", "exactly_once", "runtime_audit_required_later", "receipt_hash"], "R4 receipt");
...
function assertCreateTargetsAbsent(paths: ReturnType<typeof operationPaths>): void {
  for (const file of [paths.intent, paths.activation, paths.receipt]) if (fileExistsNoFollow(file)) fail("R4_CREATE_ONLY_COLLISION", `create-only target already exists: ${file}`);
}
...
function assertSessionBindingCurrent(binding: Readonly<{ path: string; dev: number; ino: number; prefix_bytes: number; prefix_sha256: string }>, sessionsRoot: string, sessionId: string, enforceRoot = true): void {
  assertSafeSessionIdComponent(sessionId);
  if (enforceRoot) captureTrustedSessionPrefixBinding({ sessionsRoot, sessionPath: binding.path, expectedSessionId: sessionId, prefixBytes: binding.prefix_bytes });
...
function sameFileIdentitySansRaw(left: D3V2R4FileIdentity, right: D3V2R4FileIdentity): boolean { return canonicalizeJcs(left) === canonicalizeJcs(right); }
...
function validateFrozenBinding(frozen: D3V2R4FrozenBinding): void {
  if (frozen.schema_version !== "adr0040-d3-v2-session-start-r4-frozen-execution-binding/v1") fail("R4_FROZEN_BINDING", "frozen binding schema differs");
  assertSafeSessionIdComponent(frozen.session_id);
  for (const item of [frozen.sessions_root, frozen.session_file.path, frozen.settings_path, frozen.control_root, frozen.old_activation_root, frozen.runtime_audit_path, frozen.operator_audit_path, frozen.rollback_target, frozen.quarantine_target]) requireAbsolute(item, "frozen path");
  validateD3Identities(frozen.d3_identities);
  for (const hash of [frozen.settings_pre.raw_sha256, frozen.settings_post_raw_sha256, frozen.adapter_manifest_hash, frozen.operator_manifest.manifest_hash, frozen.operator_manifest.graph_hash, frozen.operator_manifest.source_closure_hash, frozen.predecessor_dossier.raw_sha256, frozen.predecessor_dossier.self_hash, frozen.execution_dossier.raw_sha256, frozen.execution_dossier.self_hash]) assertHash(hash, "frozen hash");
}
...
function assertSandboxEnvironment(frozen: D3V2R4FrozenBinding): void {
  ...
  for (const candidate of [frozen.sessions_root, frozen.session_file.path, frozen.settings_path, frozen.control_root, frozen.old_activation_root, frozen.runtime_audit_path, frozen.operator_audit_path, frozen.rollback_target, frozen.quarantine_target]) {
...
function assertProductionEnvironment(frozen: D3V2R4FrozenBinding): void {
  ...
  if (frozen.session_file.path !== D3_V2_R4_PRODUCTION_SESSION_PATH) fail("R4_PRODUCTION_PATH", "production frozen session file path differs");
}
function productionPathBinding(): Record<string, string> { return { session_id: D3_V2_R4_PRODUCTION_SESSION_ID, sessions_root: D3_V2_R4_PRODUCTION_SESSION_ROOT, settings_path: D3_V2_R4_PRODUCTION_SETTINGS_PATH, control_root: D3_V2_R4_PRODUCTION_CONTROL_ROOT, old_activation_root: D3_V2_R4_PRODUCTION_OLD_ACTIVATION_ROOT, runtime_audit_path: D3_V2_R4_PRODUCTION_RUNTIME_AUDIT, operator_audit_path: D3_V2_R4_PRODUCTION_OPERATOR_AUDIT, rollback_target: D3_V2_R4_PRODUCTION_CONTROL_ROOT, quarantine_target: D3_V2_R4_PRODUCTION_QUARANTINE_TARGET }; }
...
function requireSourceCommit(target: "sandbox" | "production", frozen: D3V2R4FrozenBinding): string {
  ...
  const result = spawnSync("git", ["-C", repoRoot, "status", "--porcelain=v1"], { encoding: "utf8", env: { PATH: "/usr/bin:/bin", LANG: "C", LC_ALL: "C" } });
  if (result.status !== 0 || result.stdout !== "") fail("R4_SOURCE_COMMIT_REQUIRED", "production authorization requires a published clean source commit");
  const rev = spawnSync("git", ["-C", repoRoot, "rev-parse", "HEAD"], { encoding: "utf8", env: { PATH: "/usr/bin:/bin", LANG: "C", LC_ALL: "C" } });
  const commit = rev.stdout.trim();
  if (rev.status !== 0 || !/^[0-9a-f]{40}$/.test(commit)) fail("R4_SOURCE_COMMIT_REQUIRED", "clean source commit could not be bound");
  return commit;
}
