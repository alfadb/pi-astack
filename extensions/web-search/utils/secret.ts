/**
 * Secret value resolver for the web-search extension, mirroring pi's
 * own resolve-config-value semantics so a single convention works for
 * both LLM provider keys (models.json / auth.json) and the search
 * provider key (pi-astack-settings.json → webSearch.apiKey).
 *
