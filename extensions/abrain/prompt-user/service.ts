...
// Re-export shared transport types for existing importers / smokes.
export { readRouterUiEnv, type RouterUiEnv };
 ...
 *
 * Transport lives in `./router-form` (shared with vault authorization).
...
async function routerFormPath(
  ...
  const transport = await postRouterForm({
    ...
    questions: params.questions.map((q) => ({
      ...
      question: q.question,
      type: q.type,
      ...(q.options ? { options: q.options.map((o) => ({ label: o.label })) } : {}),
    })),
  ...
  if (!transport.ok) {
    handle.resolve({
      ...
      durationMs: Date.now() - startedAt,
      ...(transport.detail ? { detail: transport.detail } : {}),
    });
    return handle.promise;
  }

  // Secret redaction stays here (prompt_user lane only). Vault never
  // asks type:"secret" through this path.
  ...
  const answers: Record<string, string[]> = {};
  for (const q of params.questions) {
    const asArray = transport.answers[q.id] ?? [];
    if (q.type === "secret") {
