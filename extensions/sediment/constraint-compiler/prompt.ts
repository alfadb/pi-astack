...
export function buildConstraintCompilerPrompt(input: ConstraintCompilerPromptInput): ConstraintCompilerPrompt {
  ...
  const text = [
    ...
    "Settings/config exclusions require an input categoryHint of settings_not_memory for that exact source, or an existingDiagnostics entry for that source with SC_NOT_MEMORY_SETTINGS. Do not invent not-memory exclusions from topical words alone.",
    ...
    "For a constraint_event with categoryHint=behavioral_constraint, compile the behavioral directive or place it in unresolved[]. Do not exclude it as settings_not_memory or tool_contract_not_memory merely because it mentions config, code, bash, settings schema, removal, or checkpoint.",
