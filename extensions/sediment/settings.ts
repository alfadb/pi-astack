...
export interface SedimentSettings {
  ...
  /** ADR 0025 §4.1.5 promotion follow-up: enable multi-view gated promotion of
...
export const DEFAULT_SEDIMENT_SETTINGS: SedimentSettings = {
  ...
  /** ADR 0025 §4.1.5 promotion follow-up: default false. Enable only after
...
export const PROMPT_VERSION_NOTES: Record<keyof SedimentSettings["promptVersion"], string> = {
  ...
  outcomeSelfReport:
    "v1 (memory-footnote-v2): protocol injected via memory extension's before_agent_start hook (extensions/memory/index.ts). Main-session replies emit at most one trailing memory-footnote fence, with one entry/used/counterfactual record per memory slug and `---` between records. The collector remains backward-compatible with v1 multiple-fence output and treats repeated entry/slug fields as record boundaries; each record validates and drops independently, while unchanged valid legacy records retain their event identity. Sub-agent prompts explicitly do not write memory-footnote fences; Path A injected/implicit rows remain observation channels rather than self-report usage.",
  aggregator:
