...
export const PROMPT_VERSION_NOTES: Record<keyof SedimentSettings["promptVersion"], string> = {
  ...
  outcomeSelfReport:
    "v0: memory-footnote protocol injected via memory extension's before_agent_start hook (extensions/memory/index.ts). Not a sediment-owned prompt file; this version tag tracks the protocol-level contract: fenced self-report rows carry entry/used/counterfactual with the 3-option taxonomy; counterfactual text is required for usable self-report interpretation; sub-agent prompts explicitly do not write memory-footnote fences; Path A injected/implicit rows are split into injection-only and implicit-unused observation channels rather than treated as self-report usage.",
  aggregator:
