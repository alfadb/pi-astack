...
await check("retired taskProfile/profile fields are ignored by workflow validation", async () => {
  const withRetired = D.validateWorkflow(doc([
    agent("a0", { taskProfile: "research" }),
    agent("a1", { profile: "heavy", needs: ["a0"] }),
  ]), RO);
  assert(withRetired.ok, errsOf(withRetired));
  assert(!/taskProfile|profile=/.test(D.formatEffectiveStageLines(doc([agent("a", { taskProfile: "research" })]), { model: "x/y", thinking: "low" }).join("\n")), "effective stage lines must not advertise retired profile");
});
