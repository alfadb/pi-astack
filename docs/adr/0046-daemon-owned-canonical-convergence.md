...
- **Status**: **Accepted / implemented / Linux production accepted（2026-07-31）**（目标合同 2026-07-30 确认；D0–D5 落地；证据组合 = 文档合同 + 本地 behavior tests/gates + 真实 staged deployment + 真实 readonly aggregate-only production final state；见 pi-router `docs/acceptance/dcc-linux-production-acceptance.md`；全部 `(dcc-*)` 已勾）。**D1** Linux POSIX `0700`/`0600` attestation writer/verifier + kick/observe production installed（staged install：D1-only `62e515d…` first / D1 rollback point；full `cc22636…` third / full rollback point；`origin/release/lsea-worker-first` **contains** full，D1 为 full 直接父，二者 remotely reachable）。**Windows DCC attestation native current-primary-TokenUser protected-DACL writer/verifier pending** — 当前切片 **fail-closed**（control `unavailable` / `attestation_unavailable`；不静默依赖 inherited/default ACL）；**不**声称 Windows DCC attestation 已实现。**D2** signed daemon `0.1.160` @ `1a467ea…` / tag `daemon-v0.1.160` production ready。**D3/D4** full 已在 D2 ready 后 installed；inventory final `pending=0`/`failed=0`/`invalid=0`；stale failed intent 是用户授权删除（**不是** continuation apply 成功）；installed-full smokes 另验证 control/authority/admission/foreground-cutover。行为子路径（vault/migrate/foreground business write/dynamic mutation fence/新 pending continuation apply）由 **本地 tests/deploy-order gates** 覆盖（含 `smoke:dcc-tui-business-write`）。生产 readonly **未**触发这些子路径。**D5** observer exact ready/none + wrapper exact pass/aggregate-only。**未**声称业务写全量迁移、formal Stage A / ConsumerAck / center primary / retention 完成。
- **Date**: 2026-07-30（contract）；Linux production acceptance 2026-07-31
...
**D5 local implementation notes（2026-07-30 local；Linux production 仅验证部署顺序 + readonly aggregate 终态，2026-07-31）**：
...
- **不**授予 TUI whole-L1，**不**新增 authority；vault/secret 真正写阶段 six-gate+barrier 与 migrate-go store-present reject **local implemented**（本地 tests/deploy-order gates 覆盖，含 `smoke:dcc-tui-business-write`）；**不**声称同步 ready；生产 readonly **未**触发 vault/migrate write；installed-full smokes **不**冒充 business-write 证据。
- **Production observer aggregate CLI（local implemented + Linux production readonly exact ready/none pass 2026-07-31）**：`scripts/dcc-production-observer-aggregate.mjs` 以 jiti 加载 `observeForegroundCanonicalConvergence` + `inspectAbrainBindIntentInventory`（默认生产 deps，禁止 test hooks）；`DCC_ABRAIN_ROOT` 必须 absolute nonempty（相对路径→`absolute_invalid`；空→`env_missing`）；inventory counts 必须 `Number.isSafeInteger>=0`；stdout **sync write**（防 `process.exit` 截断）；恰好一行 `schema=pi-astack/dcc-production-observer-aggregate/v1` + `status`/`reason_code`；ready 仅六条件 ready 且 inventory 全 0；pending→`blocked`/`continuation_pending`；failed|invalid→`blocked`/`continuation_failed`；inventory 读失败→`unavailable`/`inventory_unavailable`；其它保留 closed observer reasons；stderr/stdout 不泄 path/exact secret/count；exit ready=0 其它=1。fixture smoke 覆盖 ready/pending/failed/corrupt/observer-fail/env-missing/absolute-invalid/ABRAIN leaf symlink/bind-intent intermediate symlink + zero tree delta（fixture **不是** production 证据）。**真实生产** observer exit0 exact stdout `{"schema":"pi-astack/dcc-production-observer-aggregate/v1","status":"ready","reason_code":"none"}`；详见 pi-router acceptance doc。
...
**Execution-time dynamic cutover mutation fence（本地 tests/deploy-order gates 覆盖；生产未实际行使 mutation fence）**：
