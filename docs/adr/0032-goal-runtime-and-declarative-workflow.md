...
续行消息以 user role 注入但内容是机器组装的——若被 sediment 判为 user-expressed，goal 循环就成了"assistant 文本洗白进 Tier-1 规则"的通道。双层防线：
...
ADR 0027 H5 悬置的是"hub 动态派工"：LLM 在**运行时**自主决定 spawn 什么、多少、何时停。本 ADR 的 workflow 与之有三条判别线：
