...
- sediment 去重 curator — `curator.ts:1069` `llmSearchEntries`(`status:["all"]`, `limit:5`, 手动 pin `stage1Skip=false`)
...
- ADR 0036 P6: 全局 `stage1Skip` flip 转产时, 必须在 dedup 站点**手动 pin `stage1Skip=false`**(ADR 0036 §9.1 条件 2), 否则全局 flag 会**悄悄漏进**最脆弱的去重路径(false-merge → corpus corruption)。
...
interface SearchProfile {
  ...
  searchOverrides?: (s: SearchSettings) => Partial<SearchSettings>;  // 如 sedimentDedup → { stage1Skip: false }
...
没有强制, registry 等于“加了第二种做法而 footgun 仍在” —— 与“lint + 例外”同级, 不值得做。强制后 registry > lint(lint 表达不了“当 status 含 all 时 stage1Skip 必须 pin false”这个不变量; profile 用 `sedimentDedup` 同处声明 `status:[all]` + `stage1Skip:false` 能)。
...
- **profile 钉死策略**: `sedimentDedup.searchOverrides = { stage1Skip: false }` 取代 curator.ts 的手动 pin —— 全局 `stage1Skip` flip 再也漏不进去, 因为 profile 在一处显式覆写。
...
4. **排序裁决(opus vs deepseek 分歧)**: 不 dedup-first; 可逆路径先证机制, dedup pin 已存在故最后平移(见 §6)。
5. **vs 纯 lint**: 三家认同 lint 表达不了"status 含 all 时 stage1Skip 必 pin false"这个不变量, 且会误报唯一合法的 dedup all-status 站点; **强制后的 registry > lint**, 但开放导出的 registry ≈ lint+例外, 不值得。
