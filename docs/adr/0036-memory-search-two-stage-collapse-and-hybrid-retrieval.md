...
- **条件 2 dedup pin ✅(P6 当时状态)**: `curator.ts` sediment 去重路径曾 pin `stage1Skip=false`(dedupSettings 覆写), 全局 flip 不连带翻转未验证的 all-status 近重检测。后续 P5b 已用 dedup-specific 证据解除该临时 pin; 当前 sedimentDedup 继承全局 `stage1Skip`/`sparseBM25`, 仅固定 `dedupChunk0Aggregation=true`。
...
- 调用方: `memory_search`(path-A 每轮)+ sediment dedup(每 agent_end) —— P6 切换时后者曾 pin 三阶段; 后续 P5b 已解除该临时 pin, 当前 sediment dedup 跟随读栈并保留 chunk0 聚合保护。
...
- **P3 当时纪律(后续已被 P5b 更新)**: curator dedup 路径曾 pin `sparseBM25=false` —— BM25 先只上读路径(memory_search/path-A/decide/correction), 不进脆弱的 all-status 去重。后续 P5b 用近重金标与 oracle-dedup-p5b 解除该临时 pin, 当前 sedimentDedup 继承全局 sparseBM25。
- 3×T0 执行面板(opus/gpt-5.5/v4-pro)裁定排序: 这个 ready+reversible 的读路径召回修复先于 ADR 0037 重构 ship(2:1, deepseek 主张 0037 先但认同返工成本 trivial)。P3/P6 的临时 pin 后续由 ADR 0037 profile registry 收口, 并在 ADR 0036 P5b 后按证据解除。
