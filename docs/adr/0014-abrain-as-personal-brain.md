...
> 各 phase 的 ship 状态 / commit / file:line / smoke 覆盖是**代码派生事实**，不在 docs 镜像（REQ-006：docs 不是 commit log）。
> Canonical：代码（`extensions/abrain/**`、`extensions/sediment/**`、`extensions/memory/**`）+ `package.json` 的 `smoke:*` scripts。迁移模型见 [migration/abrain-pensieve-migration.md](../migration/abrain-pensieve-migration.md)（§1 per-repo 一次性迁移）+ [migration/vault-bootstrap.md](../migration/vault-bootstrap.md)；project binding 见 [ADR 0017](0017-project-binding-strict-mode.md)，Lane G 见 [ADR 0021](0021-lane-g-identity-skills-habits-writer.md)。
> Derive：`find extensions -maxdepth 1 -type d` / `npm pkg get scripts`。
> 设计 rationale / 先例：`memory_search("abrain vault P0 七区 writer sediment B5 cutover Lane G")`。
