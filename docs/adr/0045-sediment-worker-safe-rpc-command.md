...
- LSEA worker-first overlay: once the canonical ABRAIN authority store exists (`held|draining|free|corrupt|unavailable`, including free+unlocked), ordinary Pi is capture-only regardless of the legacy effective-owner fallback. Only a completely missing authority store retains legacy settings-based execution for this rollout slice. Full details: [Local Sediment Executor Authority worker-first admission](../architecture/local-sediment-executor-authority.md).
...
**Current-candidate unfinished artifacts (fail closed)**: when the **current** task-scoped candidate produces multiview pending / staging deferred / promotion-needed unfinished artifacts, the worker **must** return exact `current_candidate_deferred` (`retryable=true`, no poison) — **no CP advance, no success receipt, not processed**. Detection uses **only this run's internal outcome keys** (e.g. `multiview_staged_for_replay`, curator `multi_view.staged`, this-run classifier `stagingWritten` on long-window / explicit / short-window paths) — **never a global staging/multiview directory scan**. On `stagingWritten=true`, mark attempt deferred immediately and stop subsequent candidate/drain; that attempt must not save CP (short-window keeps capture/terminal-reject exception). Mark/consume is **attempt-local** (`AsyncLocalStorage` per agent-end pass) — **not** a process sticky session map; exception / deadline / next terminal / independent worker task for the same session must not inherit the flag. On deferred, **stop this-task recursive candidate/drain immediately** so later outcomes cannot advance CP; future independent worker tasks still drain normally. Global resolver/replay remain skipped in Stage0. **Candidate-scoped resolver/replay is a future slice**; until then unfinished current-candidate artifacts fail closed as `current_candidate_deferred`.

**Retryable return invariant (receipt + CP re-read)**: before returning any post-pass retryable exit (`current_candidate_deferred` / `no_progress` / deadline-class / `pipeline_threw` / …), worker **must** re-read the create-only processed receipt and durable CP. Valid receipt → settled success. Deferred uses attempt-wide `any_advance` against the **attempt baseline** CP (before first pass) plus `anyAdvanced` from prior more-loop iterations — partial advance then deferred is fatal poison, not retryable. Covers-tip without receipt → fatal `deadline_after_checkpoint_advanced`. Post-pass CP re-read failure → `checkpoint_state_unknown_after_pass` (nonretryable + restart/poison; cannot authorize retry). Soft-deadline fence **must preserve** first-cause `receipt_write_failed` / `checkpoint_state_unknown_after_pass` (not rewrite to `deadline_after_checkpoint_advanced`). Normal deferred **requires** CP not advanced this attempt. Post-CP audit is best-effort and must not reverse durable CP success.
...
| `deadline_after_checkpoint_advanced` | durable CP **covers sidecar tip** (or this-attempt any advance under deferred/pipeline invariant) but **no** success receipt — fail closed, human diagnosis; not auto no_progress / already_processed | **false** | true (poison) |
| `checkpoint_state_unknown_after_pass` | post-pass CP re-read failed — cannot prove/disprove advance; **must not** authorize retry | **false** | true (poison) |
| `worker_process_poisoned` | process already poisoned; refused claim/pass | true | true (poison) |
| `current_candidate_deferred` | task-scoped current candidate left unfinished artifact (multiview pending / staging deferred / promotion-needed / this-run `stagingWritten`); this-run keys only; **CP must not have advanced this attempt** | true | **false** (no poison) |
| `receipt_write_failed` | create-only receipt write failed/timed out under hard reserve after more=false main-chain success (**CP already advanced**); deadline fence must **not** rewrite this first cause | **false** | **true** (restart_child; first cause preserved; optional atomic-write retry inside hard reserve only) |
...
- **Canonical ownership** (historical Stage0 worker-internal mechanism; **migration pointer** → [ADR 0046 DCC](./0046-daemon-owned-canonical-convergence.md)): process-level startup attempt is created/retried in `runOutsideWorkerBudget` (outside task ALS; full busy budget; not bound to short task deadline). RPC tasks never wait 60m — if process-level startup is not already ready, return cooperative deferred (`STARTUP_BUDGET_EXHAUSTED`) immediately (retryable/held, no poison). Each external task best-effort kicks the next generation; bootstrap continues for worker process lifetime. Worker shutdown/restart drops in-memory attempt; a new process re-bootstraps. Does **not** hard-kill in-flight mutation. CE/writer share the process runtime singleton. **DCC target**: under store-present strict regime, canonical convergence executor ownership moves to the same LSEA holder's daemon-managed long-lived worker via daemon first-class kick/observe + sibling attestation; TUI whole-L1 startup is removed; this subsection remains the historical Stage0 contract and is not silently rewritten as already-cutover.
...
- Worker-first rollout adds strict single-entry LSEA admission (original LSEA baseline) without duplicating legacy B1-B8 write barriers; DCC ADR 0046 overlay separately revalidates on canonical mutation frames; full dual-executor exclusion still depends on pi-router's long-held lock and process containment
