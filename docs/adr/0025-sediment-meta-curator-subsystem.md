...
- **DEFER 后的归宿**：reviewer DEFER 时，那个 op 是写 staging 还是直接丢弃？**默认写 staging**（与 §4.1.5 attribution_pending 同款路径），避免信号丢失 → **✅ AMENDED (2026-06-08, commit `c918c8b`)**：对 user-expressed Tier-1 指令（`shouldEscalateToCurator`：quote_source=user_message ∧ provenance=user-expressed ∧ durable ∧ conf≥8）DEFER 时**不再写 staging**，改为确定性提交 proposer 决策（`multi-view.ts:1163`），杜绝 defer→replay→`terminal_max_retries` 静默丢失；非指令候选仍按默认写 staging。
...
batch 3 全系列 (commits `3718d91` → `1a6f6f7`, 11 commits) 实施§4.4.5 · "DEFER 默认写 staging" 的全面推广：
  ...
  - 6 条 transient fallback path 都走 staging（`reviewer_unavailable` / `pass1_call_failed` / `pass1_unparseable` / `pass2_call_failed` / `pass2_unparseable` / `deferred`） → **✅ AMENDED (2026-06-08, `c918c8b`)**：`deferred` 对 user-expressed Tier-1 指令例外——确定性提交而非 staging（见上「DEFER 后的归宿」注释 + `multi-view.ts:1163`）；其余 5 条 transient path 及非指令 `deferred` 不变。
  ...
  - Replay terminal 走 “op=skip + 删除 + audit”（不依赖 §4.6 archive-reactivation prompt） → **✅ AMENDED (2026-06 机械门控 sweep)**：机械 terminal 路径（retry-cap / stale / trigger-disappeared）已从硬删除改为**软归档到 `abandoned/`**（`multiview-staging-replay.ts` `archiveTerminalOrAudit`），保留候选可追溯；仅 reviewer 主动决定的 skip 仍删除 staging 行。
