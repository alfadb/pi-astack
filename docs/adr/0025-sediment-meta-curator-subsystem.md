...
| invariant | 代码覆盖 | 评估 |
|---|---|---|
| INV-INVISIBILITY（不阻塞主会话） | ✓ Lane C fire-and-forget @ `index.ts:1035`、agent_end 不 await bg | 已实现 |
| INV-INVISIBILITY（用户不被要求做管理事务） | △ 部分 | Lane C 不问用户（OK）；但 `/about-me` slash + `MEMORY-ABOUT-ME:` 围栏仍要求用户主动声明（反模式），过渡期保留 |
| INV-INVISIBILITY运行状态指示 | ✓ footer / notify | 运行状态正常显示让用户明确感知大脑在工作——2026-05-24 误删后已恢复 |
...
| staging 里的 provisional hypothesis **不能**在没有 multi-view 审查的情况下自动升成 durable entry | §4.4 multi-view verification。这条放在这里是硬性规定：升格前必须过一道跨 provider 双审。如果 multi-view 挂了或没启用，provisional 只能保持 staging 或 age-out |
...
**footnote 丢失怎么办**：`memory-footnote` 是藏在 response 末尾的 fenced block——如果用户发了下一条消息前 LLM response 还没走完（被截断）、或者 session crash、或者 LLM 忘了写，sediment 在 `agent_end` 汇总时就看不到某些 entry 的 footnote。分三种情况处理：
