...
- **P7 非-active 查询 stage0 化(2026-06-14, 4×T0 设计 review, supersede 本节 P3 修订 7)**: 修订 7 假设“非-active-status 查询罕见+条目少, 回退全 corpus 可接受”被 **sediment curator 去重 search** falsify: 它每轮 agent_end 对每个 candidate 调 `llmSearchEntries(..., filters:{status:["all"]})`, 触发 selectStage0Pool 的 `wantsNonActive→return null→回退全库 full_body`, 大库 1144 entry 每次 915K token 喇 flash(cache=0), 是高频稳态成本(实测占当日 flash 大请求主体)。修复(4×T0 一致): (1) 删 `wantsNonActive→null`, 非 active 查询走 hybrid 缩候选(dense 在 active 索引[非 active 不在索引自然不返回] + sparse 扫全 corpus[非 active 唯一召回通道] + stale); (2) **load-bearing fix**: staleOrMissingSlugs 只对“可索引集”(`status==="active"` 且 `frontmatter.zone !== "rules"`)算——否则 status:all 把所有非 active + readonly rule neighbors(zone:rules, 不经 loadEntries 永不被 reconcile embed)标 stale 塞爆候选池(永久 stale)。非 active 召回靠 sparse(够用; 漏召代价中等可自愈; **不** embed 非 active=方案 C, 因索引膨胀/prune 复杂度/archived 噪声 而 defer)。验证: `smoke-stage0-nonactive`(status:all 不回退全库 + 55 不可索引 probe staleCount=0 + 相关非 active 旧版 sparse 召回成功) + oracle active coverage 100% 无回归。**verdict=none 对非 active 查询仍 pool-relative**(与修订 9 一致)。遗留 backlog: (a) sparse tokenizer 对中文非 active 召回弱(kimi 指出, 影响中文 entry 去重); (b) guard(sediment 路径 full_body+大 corpus 报警/fail-closed 防复发); (c) sediment dedup oracle slice(记录 supersede/merge target 是否在 pool, 检测非 active 召回回归)。
