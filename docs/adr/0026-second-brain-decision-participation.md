...
> 🗄️ **机制已 ingest 入 abrain（pi-global）@627de33**：本 ADR 的机制 rationale 已由 ADR 0034 ingest lane 分解为 **12 条 typed entry** 存入第二大脑，逐条 rationale 经 `renderRationale` 可得（带 pinned `source_ref` SHA）。方向契约见 [`direction.md`](../direction.md) / [`requirements.md`](../requirements.md)。原机制 prose（402 行全文）见 git `@627de33`（`git show 627de33:docs/adr/0026-second-brain-decision-participation.md`）。
