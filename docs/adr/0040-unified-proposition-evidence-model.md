...
- **Implementation status**: 当前实现与未完成门只以 [Current State](../current-state.md) 和 [Transition Register](../transition-register.md) 为准。
...
`session_start` 不调用 LLM，不扫描原始 L1 event 集合，也不在启动路径实时裁决命题是否应注入。`session_start` 只读取上一份已经生成并验证的 stable policy / constraint view，以及明确的 queued / stale / diagnostic 状态；若 view stale，启动路径只能按既定失败语义展示状态或使用已授权的上一版稳定 view，不能同步触发 LLM 综合。
...
No-migration cutover 不等于当前 runtime 已经切换，也不等于旧 runtime 立即退休。它只规定未来新 normative projection 的权威来源：从 genesis/cutover 后的新 L1 evidence 开始，旧材料只作 cold audit，不作新 view 的初始事实集。
...
每个读取面必须独立提供证据、失败语义、回退方案与 fresh authorization；一个读取面的 cutover 不授权其它读取面。
...
分阶段实施会延长过渡期，并要求同一系统同时存在旧 runtime、shadow projection 和新 schema 设计。接受该代价，因为它防止 ADR acceptance 被误用为生产切换授权，并允许每个读取面基于真实数据独立验证。
...
如果 `session_start` 调用 LLM、扫描原始 L1、即时裁决注入资格或在 stale 时同步编译，说明 push read contract 被破坏。如果 LLM 综合结果未先固化为 L1 projection event 就直接生成 stable L2 或 runtime view，说明确定性 renderer 边界失效。如果 lifecycle event 进入 compiled behavior，或目标 L1 被修改、删除、重写，说明 immutable SOT 边界失效。
...
用户随后修正并 ratify 的关键边界为：现有所有 rules、constraint evidence、compiled rules 不迁移、不转换、不自动激活进新的 normative projection；旧材料只可作为 cold audit history 保留，不再拥有新 policy view 的 runtime authority；新 policy view 从明确 genesis/cutover 边界开始；不得为兼容旧规则设计迁移层；统一 Evidence 架构是目标但必须分阶段实施，ADR acceptance 不授权代码切换，每阶段需要真实生产证据与 fresh multi-T0 一致授权。
