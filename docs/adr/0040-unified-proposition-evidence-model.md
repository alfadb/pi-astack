...
- **Implementation status**: 2026-07-20 production full-flip 代码、配置清理与 sandbox acceptance 已实现；真实 production target 因旧 smoke 在授权门移除后继续调用 production mode 而被意外 materialize，正式 main-session acceptance 与 runtime restart 仍待完成。当前事实以 [Current State](../current-state.md) 为准。
...
每个读取面必须独立提供证据、失败语义与 fresh authorization；一个读取面的 cutover 不授权其它读取面。2026-07-20 用户对 Policy/session-start push 读取面给出直接 full-flip 授权，并明确无回退，因此该读取面的终态失败策略是 loud zero injection，而不是恢复任何旧 authority。
