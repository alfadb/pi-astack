...
> **实施状态 (2026-05-28)**：heartbeat writer-side ✅ **Stage 1b 已落地**（commit chain `ce1f00f..9796bdd`，1 轮 R8 盲审后 unanimous GREEN）。
>
> - 主模块：`extensions/_shared/heartbeat.ts`（55 行）
> - dispatch 集成：`extensions/dispatch/index.ts` `runInProcess` 决定过 (try/finally 覆盖所有 early-return 路径)
> - **已落地能力点**：HeartbeatHandle、globalThis singleton (jiti 多副本防收）、file-based JSONL writer 于 `.pi-astack/dispatch/heartbeat/<anchor>.jsonl`、phase taxonomy (started / alive / stopping)、schema_version: 1 + seq monotonic + interval_ms (让 consumer 不必 hardcode)、R8 stop() 顺序修复 (writeOne stopping before stopped=true)、finally 释放 (timer/file/registry 三者同步 unlink)
> - **已推迟项**：Stage 1c consumer (从 heartbeat trace 推导 terminal_state=cancelled + cancel_source=timeout) 推迟。该阶段可能需要 audit row v3 加 `heartbeat_trace_path` 字段或明确声明 derived-from-anchor 契约。同时，v1 是 always-on heartbeat（不是 spec 表要求的 “caller timeout > 50% 才必须”）——over-implementation，forward-compatible。
...
> **实施状态 (2026-05-28)**：✅ **Stage 1a terminal_state schema 已落地**（commit chain `dab011b..c69f8f8`，3 轮 R6→R7.1 盲审后 unanimous GREEN）。
>
> - 主模块：`extensions/dispatch/terminal-state.ts`（349 行）
> - dispatch 集成：`extensions/dispatch/index.ts` audit row v1→v2 升级
> - **已落地能力点**：terminal_state taxonomy (completed / failed / degraded / cancelled)、严格 scope (degraded 不携带 cleanup_done，failed 不携带 what_dropped等)、materializedResults 稠密数组补洞、aggregate cancelSource override、single task degraded 错误限制、tool_rejected audit gap 裥上、v1 全部 `resumable: false` 显式标记
> - **已推迟项**：resume 语义 (`resume_from_checkpoint` + `idempotency_key`) 推迟到 v2。audit row v3 加 `heartbeat_trace_path` 推迟到 Stage 1c consumer 同期。
