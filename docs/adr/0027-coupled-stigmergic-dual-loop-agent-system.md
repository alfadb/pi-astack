...
**2026-07-20 awaited lifecycle liveness clarification**：pi 会 await `agent_end` extension handlers，且 `agent_settled` 同样是 awaited lifecycle surface。L1 的分钟级 canonical recovery、classifier、curator、projector 或 Git history validation 不得占用这两个 handler；handler 只能同步捕获 plain session/event/anchor 输入并投递 process-level multi-key queue（同 key 串行、跨 key 有全局并发上限；同 repo 写仍 resource-lock 串行）。队列 catch+audit rejection（含 `--unhandled-rejections=strict`）、不得保留旧 session UI/ctx；canonical startup 未完成时，同 session 多次 `agent_end` 可以 coalesce 到最新 full-branch snapshot。durable checkpoint v3 记录 branch lineage + candidate idempotency：same-lineage compaction 可 oldest 重放，fork/unproven legacy fail-closed 不前移。主 lane 与 drain 对齐——terminal durable 成功/terminal duplicate 的 MEMORY/ABOUT-ME candidate 立即记 `processedCandidateKeys`，即使同窗另一 candidate transient 失败导致 watermark 不推进；partial key 保存不推进 `lastProcessedEntryId`。ABOUT-ME staging basename 仅由稳定 session/source/content hash 派生（无 wall-clock），跨日同 candidate `staging_idempotent`。checkpoint 未追上冻结 tip 时 queue job ready-pending 续排。`waitUntilReady=false` 必须 park（count/bytes/TTL 有界），production onReady wake；blocked startup 可重试 eviction。`correctionPromise` 与 window-bound child work 按 session/resource key 跟踪，wait 不得跨 key 互挡。Cold-start 不可变 classification 在 mutation barrier 外：`headBefore→scan/status→headAfter` 冻结，获锁后重检，freeze 异常也进 barrier 判定，漂移有界重算。
