...
> 2026-06-16：[ADR 0030](0030-l2-hub-baseline-and-evaluation-harness.md) 已起草并 owner-ratified 为 baseline——单用户 owner-consent 下以在线评估器（生产 audit + 主会话处置 + 周期性跨厂商双跑判定）定义 harness，caged-live hub 以默认关闭 flag 进 dogfood。本节保留 gate 的原始论证（仍是判别基准）。
...
**ADR 0030 启动条件（mechanical trigger）**：任何提案若让系统在运行时由 LLM 自主决定 spawn 哪些 worker、worker 数量、角色分工、拓扑增删，或把当前受禁的 spawn-class tool（`dispatch_agent` / `dispatch_parallel` / 同类工具）放进可生产运行的 workflow stage，即触发 H5 gate，必须先起草并接受 ADR 0030；静态 DAG workflow、线性 goal auto-continue、固定 reviewer 名单 multi-view 不触发此 gate。
