...
- **范围**：CSDLAS 是 pi 内 multi-agent 系统的总框架。不直接定义任何具体能力点的实现；具体能力推到 ADR 0038（IDE/host 边界）/ ADR 0029（跨 provider 治理）/ ADR 0030（hub baseline + evaluation harness）。
- **编号更正**：IDE/host 边界子 ADR 原规划为 0028，但 0028 已被 [Sediment Ground-Truth-Tiered Rearchitecture](0028-sediment-ground-truth-tiered-rearchitecture.md) 占用，故重新编号为 **0038**；0029/0030 仍空缺有效。本文余下对该子 ADR 的引用均已改为 0038。
...
- 任何 ADR 0029/0030/0038 的具体设计——子 ADR 启动后另写
...
Heartbeat 是 L2 liveness 通道，不是业务 trace 本身。Writer 必须输出 `started / alive / stopping` phase、`schema_version`、单调 `seq` 与 `interval_ms`，consumer 才能在不 hardcode 采样间隔的情况下从 heartbeat trace 推导 `terminal_state=cancelled` 与 `cancel_source=timeout`。如果实现选择 always-on heartbeat，也必须保持与“预计执行 > caller timeout window 50% 时必须写 heartbeat”的规范兼容。
...
Terminal-state schema 是 L2 问责边界：每个 task 必须能被归入 `completed / failed / degraded / cancelled`，且不同终态携带的字段必须严格分区，避免 downstream consumer 把 degraded 当 failed、把 cancelled 当业务失败。Resume 语义必须显式声明 `resume_from_checkpoint` 与 `idempotency_key`，或显式声明 not-resumable；heartbeat trace path 应与 terminal_state 因果锚点可 join。
...
**与 ADR 0024 §3.3 的反向补丁**（R1 P1-5，2026-05-27 补补）：ADR 0024 §3.3 "机械 vs AI-Native 对照表" 列举五个维度都是**认知层**（classifier / staging / scope / audit 可信度 / writer 还原）。multi-agent 协调（L2 调度 / cancellation / heartbeat / done-marker schema）是**infra 层**，应该作为表的第六行明确列出，防止未来 reviewer 误读 §3.3 那些机械机制不可用。C3' 已明确：认知层 prompt-native，infra 层 structured，两者不冲突。其潜在 patch 到 ADR 0024 §3.3 的行：「**multi-agent 协调与交付** · 机械形态禁用 (怎么理解成认知层) · AI-Native 边界 (infra layer structured retry counter / cost accounting / cancellation token / done-marker schema 都 OK，详 ADR 0027 §C3')」。
...
**Anchor canonical lifecycle owner**（R1 P1-4，2026-05-27 补补）：`dispatch` extension 是 anchor 生命周期的唯一 owner。其 `activate(pi)` 调用 `bindCausalAnchorLifecycle(pi)` （`extensions/_shared/causal-anchor.ts`）在 main pi + shared 子代理 loader 两个 runtime 各装一次。**禁用 dispatch extension 等于禁用 C6 anchoring**——所有 audit writer 会写 `spreadAnchor(undefined) = {}` 空 anchor 行。这是隐式依赖，不可以郁 silent：未来将 anchor lifecycle binding 外提到独立 `_shared` 启动模块（不依赖任何单个 extension）可以消除这个 single point of failure。R1 现阶段 "dispatch 必用 → anchor 一定存在" 是可接受的过渡状态。

**2026-07-08 更新注记（walkback 理由：loader 与 live 事故证据推翻外提启动模块假设）**：外提 `_shared` 启动模块方案经评估不可行。pi loader 给每个扩展独立模块实例，handler 按 per-extension 注册序触发；独立启动模块无法保证 memory 自身的 turn bump 先于其 Path A reader。旧的 process-wide 注册守卫曾造成 live `anchor_missing` 事故，证据见 `extensions/memory/index.ts:345-357` 注释：当 dispatch 先绑定时，memory 的 bind no-op，bump 落在 dispatch 的 pi 上并晚于 Path A reader。现行 multi-bind + fire-time 幂等是事故校准后的正确形态：每个相关 extension 都注册 lifecycle，重复递增由 `causal-anchor.ts` 的 per-turn flag 在触发时防止。

**2026-07-20 awaited lifecycle liveness clarification（queue readiness 部分已于 2026-07-23 supersede）**：pi 会 await `agent_end` extension handlers，且 `agent_settled` 同样是 awaited lifecycle surface。handler 只持久化轻量 create-only intake receipt；classifier、curator、projector、Git 或 canonical startup 不占用 awaited handler。process queue 只保留同 key latest coalesce/串行、跨 key cap、`more` continuation 与 rejection containment；restart recovery 由 durable intake 负责，旧 wait/park/wake/TTL/bytes/readyPending lifecycle 已删除。durable checkpoint v3 继续记录 branch lineage + candidate idempotency；terminal durable candidate key 可部分保存而不推进 watermark。ABOUT-ME staging basename 仍由稳定 session/source/content hash 派生。`correctionPromise` 与 window-bound child work 按 session/resource key 跟踪，wait 不得跨 key 互挡。Cold-start 不可变 classification 在 mutation barrier 外的既有约束不变。

**2026-07-21 concurrent startup liveness clarification**：cold-start 采用显式 phase machine：`freeze/classify initial（锁外）→ journal bootstrap mutation（锁内）→ recovery classify（锁外）→ recovery/index/backlog mutation（锁内）→ final classify（锁外）→ tuple stable publish-ready（锁内）`。每次锁内阶段只消费并复验前一锁外阶段冻结的 `HEAD + whole-L1 scanRoot + statusHash`；漂移必须释放 barrier 后独立有界重算，未通过 final tuple 复验或仍有 open/quarantined recovery 时不得发布 ready。仅 `CANONICAL_MUTATION_BUSY` 在 startup runtime 层按 monotonic 总预算重试（生产默认 10 分钟，单次 barrier timeout 仍为 30 秒），每次 busy retry 从 fresh freeze 开始，使用单 shared promise/单 timer 的 exponential backoff+jitter；预算耗尽解析为 typed `deferred + retryable` diagnostics、逐出 promise、无自动 timer，等待后续 `session_start`/`agent_end` 等外部 lifecycle 重新触发。未知错误仍 reject fail-closed；同 startup key 的终态 rejection 最多一条 generic error，consumer continuation 始终从动态最新 reporter 交付。底层 OFD probe 同样改为 capped exponential backoff+jitter，保留单调用 timeout 契约。

**2026-07-23 C6 frozen publication clarification**：event-first outbox publisher 的 ownership unit 是一次 bounded frozen batch，不是单 item。取得一次 OFD 后冻结 ready receipts（普通目标 <=64，merge 不拆；单原子组 >64 可单独完整 freeze）与 HEAD；projection closure 只能是 frozen HEAD tree 中 registry-validated Knowledge L1 加本 batch validated L1。HEAD 必须通过 structured Git tree/blob APIs 读取，worktree 上未发布 sibling/tail 不得进入 fold、manifest 或 exact cohort。pure projector 对 closure 一次 fold，exact cohort 同时包含 batch L1、affected L2 output/removal 与 manifest，并以 temp index/ref-CAS/index convergence 一次收敛。HEAD L2 watermark/manifest 指向非 HEAD L1 时，同 closure 自动重建；无 receipt 的显式 one-shot 可 repair-only。只有 exact commit（或 HEAD-byte no-op）与 index convergence 完成后 receipt 才可 ack；CAS 后 ack 前 crash 由下一次 replay 收敛。OFD busy/CAS/index failure 与 detached/unborn/symbolic-ref 暂不可用均不 ack、不进 failed；仅 content/schema/identity/path 确定性 terminal 进 failed。Knowledge receipt 携带 durable intake `windowId`，publisher 仅在该 exact window 仍 pending 时 hold（legacy 无 windowId 才 session fallback）。Publisher 不消费 shared startup recovery、不自动循环、不隐式 push。确定性 writer reject 不得被全局 reason 前缀改写；既有 terminal taxonomy 允许 checkpoint 前移，只有实际 transient append/outbox/merge-receipt failure HOLD。生产 repair-only frozen-publisher 证据已 accepted（见 `docs/evidence/2026-07-23-sediment-production-acceptance.json`）。
...
**触发条件**：单设备 dogfood 推到团队/企业场景，或 multi-view 治理真正接入跨 provider reviewer。

---

### ADR 0030（**历史 blocking gate；当前无 hub 客体**） — Hub baseline + evaluation harness

> 2026-06-16：[ADR 0030](0030-l2-hub-baseline-and-evaluation-harness.md) 曾起草并 owner-ratified 为 baseline——单用户 owner-consent 下以在线评估器（生产 audit + 主会话处置 + 周期性跨厂商双跑判定）定义 harness，caged-live hub 以默认关闭 flag 进 dogfood。本节保留 gate 的原始论证（仍是判别基准）。
>
> **2026-07-22 walk-back**：`dispatch_hub` 已立即退役，ADR 0030 现为 superseded/retired，当前 runtime 不存在动态 hub/planner 客体。旧 baseline 不再清除 H5，也不构成恢复授权；任何未来动态 hub/planner 恢复都必须由 owner 作出新决策、通过架构审查并另起新 ADR，届时 H5 gate 从恢复进入 runtime 之前重新生效。以下历史论证保留，不应读成当前存在待放量的 hub。
...
**历史 ADR 0030 启动条件（mechanical trigger）**：任何提案若让系统在运行时由 LLM 自主决定 spawn 哪些 worker、worker 数量、角色分工、拓扑增删，或把当前受禁的 spawn-class tool（`dispatch_agent` / `dispatch_parallel` / 同类工具）放进可生产运行的 workflow stage，即触发 H5 gate。2026-07-22 walk-back 后，满足该 trigger 必须起草并接受**新的**恢复 ADR、取得 owner 决策并通过架构审查；不得重新接受或复用已 retired 的 ADR 0030。静态 DAG workflow、线性 goal auto-continue、固定 reviewer 名单 multi-view 不触发此 gate。
...
3. **C3' 认知/infra 边界在实际 PR review 中频繁出现争议**（"这个 done-marker schema 算认知还是 infra？"）→ §4.2 三态标注需更细化；或在 ADR 0029/0030/0038 各自补充本子领域的判别例子
...
- **不是** "完整生产设计"——ADR 0030 evaluation harness 未完成前 L2 swarm 不进生产（§6 / §8 代价 3）。本 ADR 是 baseline framework，不是 production-ready spec。

---

## 11. 评审来源（provenance）

本 ADR 的 v0.1 → v0.2 由三轮跨 provider T0 盲审（Claude Opus / GPT-5.5 / DeepSeek v4-pro）收敛而来：从 "Sympoietic Two-Tier Swarm + 四论断" 收敛为 **Coupled Stigmergic Dual-Loop Agent System + 六论断 C1'-C6'**（framing 2/3 多数，Opus 接受为 baseline 并保留意见）。立场迁移矩阵、逐轮转录、评分变化（工程可落地 5.0→8.0）见 [`docs/audits/2026-05-27-adr-0027-csdlas-r1-r3.md`](../audits/2026-05-27-adr-0027-csdlas-r1-r3.md)，不并入本正文。

纳入决策的 reviewer 保留意见（已落为 gate，勿丢）：
- **C5 必须先于 L2 用户主路径落地**——"先上 L2 再补 fail 语义" 的 PR 应 block（见 §2 C5 + §8 代价）。
- **ADR 0030 evaluation harness 是 L2 进生产的 blocking gate**（见 §6 ADR 0030 / §8 代价 3）。
- **P1 identity drift 必须在 L2 主路径前决断**（见 §7 P1）。
