...
> **Retired / superseded（owner 决策，2026-07-22）**：立即退役 `dispatch_hub`，不再继续 20 条 dogfood。退役主因是实际采用极低，以及 `task -> hub -> worker` 相对主会话直派形成结构冗余和两级有损转述；主会话直接使用 `dispatch_agent` / `dispatch_parallel` 已足够表达所需编排。仅有的一条 material verdict 为 `human_better`，它是与上述判断一致的 n=1 佐证，不是统计结论。专属 runtime、settings schema/config 与继续推动 dogfood 的评测入口已删除；历史 ADR、audit 与 ledger 作为事实证据保留且不再增长。

- **状态**：**Superseded / retired（2026-07-22）**。**Accepted (baseline，2026-06-16)** 仅是历史状态：当时经三轮跨 provider T0 设计盲议与 owner 签字，允许默认关闭的 caged-live hub 进入单用户 dogfood；它不再是当前能力或恢复授权。
- **依赖（历史 baseline）**：[ADR 0027](0027-coupled-stigmergic-dual-loop-agent-system.md)（CSDLAS / C1'-C6 / H5 gate）、[ADR 0009](0009-multi-agent-as-base-capability.md)（dispatch 底座）、[ADR 0024](0024-second-brain-from-natural-conversation.md)（AI-Native 认知/infra 分层）、[ADR 0026](0026-second-brain-decision-participation.md)（§3.4 回声室断路器）、[ADR 0028](0028-sediment-ground-truth-tiered-rearchitecture.md)（provenance 来源边界门）、`docs/direction.md`（INV-COST-NOT-A-GATE / INV-IMPLICIT-GROUND-TRUTH / INV-USER-NOT-WORKER）。
- **H5 关系**：本 ADR 曾仅为已退役 hub 清除 ADR 0027 §6 的 H5 gate。退役后当前无 hub 客体；任何未来动态 hub/planner 恢复都须新 owner 决策、架构审查和新 ADR，H5 重新生效。

> **历史读取规则**：以下 §1-§11 保留 2026-06-16 accepted baseline 与后续 dogfood walk-back 的原始论证。其现在时描述、flag、脚本与源码路径均不得当作当前可执行 surface；已删除入口只可从 git history 查阅。
...
- **轴 2 正确性（去相关，不可凭 hub/主会话自证）**：
  ...
  - **(b) material 模式离线双跑判定**：历史入口 `scripts/oracle-hub-quality.mjs`（已删除，见 git history）曾产出 human-pick / hub-pick 候选材料，主会话再用 `dispatch_parallel` 组织跨厂商 T0 盲判哪个 assignment 更好。**这是当时唯一真正的"不一致谁对"信号**。
...
**2026-07-21 历史实态注记**：`hub-judgments.jsonl` 当时为 **1/20** material 判定，且该条 `final_verdict=human_better`（2026-07-08）。该 n=1 结果只构成佐证，不支持统计性质量结论；2026-07-22 的退役主因仍是低采用与结构冗余。2026-07-15 “至少 1 次真实判定” 条件当时已满足，但默认开质量门未满足。历史结构只读 cage 源码入口 `extensions/dispatch/hub.ts` 已删除，需从 git history 查阅。
