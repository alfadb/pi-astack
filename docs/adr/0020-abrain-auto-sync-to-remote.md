...
非 noop join 的 fetch、tree 计算和 push 不持 OFD 锁；同 OID fast path 完全不进入 OFD 发布边界。发布进入锁后先恢复既有 device-join journal、恢复 canonical local-drain journal并排空合法 pending canonical 写入；如果 HEAD 已非 `H`，释放锁后重算。写 journal/CAS 前验证完整 `H -> M` delta 的每条 path 在 **worktree 与 shared index** 上都是 exact `H` 前像或 exact `M` 后像（第三态分别 typed 为 worktree/index）；普通 create 只接受 absent 或由完整 delta 证明可移除的合法目录。delta 外 **ordinary** staged/untracked/`.gitignore` dirty 原样保留，不要求 whole-tree clean；该保留域仍要求路径为 canonical Git path encoding（与 `decodeCanonicalGitPath` 一致：合法 UTF-8 NFC、正斜杠分段、无 `\\`/绝对盘符/`.`/`..`/空段），noncanonical path 在 index/tree 解析处 typed fail-closed，不扩展 parser 去接纳。delta 外 **canonical** 例外：tracked/staged modified/deleted L1 与注册 L2 必须 fail-closed；untracked valid L1 backlog 可保留；unknown registered L2 仍 fail-closed。唯一迁移例外是 reconciler-owned registered L2 的旧 untracked+ignored leaf：CAS 前只 plan，物理 unlink 推迟到 CAS 成功且 HEAD 已为 `M` 后的 materialize，避免 stale CAS 留下永久缺失 tracked leaf；candidate 必须删除旧 `l2/views/knowledge/latest/manifest.json` ignore 行。journal 绑定完整 tracked delta；CAS 后先按深度移除删除项以关闭 directory/file blockers，再物化 L1 create-only、注册 L2 全量重建结果和普通 tracked add/replace/mode update，最后只对 delta 做 `update-index` 收敛并立即 readback exact `M`（删除零 OID 按 repo object format / entry oid 长度支持 40/64），并对 delta 外 shared index（含 stage-0 与 multi-stage ordinary）做 fingerprint 不变断言。写 journal/CAS 前对每条 `after!=null` 的 delta path 纯校验既有 parent 为真实目录（`assertExistingParentsSafe`），阻止 untracked file 占父路径在 CAS 后 wedge。恢复与 publish 同边界：每条 journal path 的 worktree/index 须为 exact `H`/`M`，unknown registered L2、canonical outside-delta dirty 与 unsafe parent 在 CAS 前 fail-closed；只可删除名称绑定 journal M blob、类型/owner/link count及内容前缀验证通过的本协议 atomic temp。清 journal 前验证 `HEAD=M`、delta index/worktree exact `M`、candidate `.state/` contract，以及对 M 中全部 L1/注册 L2 的 index+worktree exact 不变量（不是 tree 对自身的恒真断言）。noncanonical Git path、dirty directory replacement、不可物化 symlink target和 changed gitlink 均在 journal/CAS 前 typed fail-closed。
...
`npm run smoke:abrain-device-join` 覆盖 clean divergence、L2 manifest 重建、旧 ignored+不同 bytes manifest 首次迁移与 stale-CAS 不预删、普通 ignored create pre-CAS 拒绝、普通 tracked add/modify/delete/mode 与 directory/file transition、delete recovery index 收敛、L1 修改/删除/同路径异 blob 拒绝、普通双侧冲突、`.state/` contract、gitlink pre-CAS rejection、journal 各崩溃点与 validated atomic temp 恢复、delta 外 ordinary dirty 全量 status 保留、delta 外 stage-0/multi-stage ordinary index fingerprint 不变、create parent 被 untracked file 占用时 publish/recover 均 pre-CAS `DEVICE_JOIN_WORKTREE_PARENT_UNSAFE` 且 HEAD/journal 边界正确、delta index 第三态拒绝、recovery 期 unknown L2 pre-CAS HEAD 不动、outside tracked canonical L1 dirty 拒绝、untracked valid L1 backlog 保留、跨进程 OFD 互斥、长编译不阻塞 writer、startup timeout 可重试、legacy writer 与 join 互斥、detached-context lease invalidation、CAS 各窗口 race 与有界 push retry。`npm run smoke:abrain-git-sync` 覆盖真实 bare remote 的 writer `fetch -> join -> exact-OID push`、behind fast-forward 与 divergence、同 tip 6000-file 仓库在 prepare/barrier fault injection 下的 typed fast noop 和 3s 上限、真实 canonical `session_start` 先 drain dirty L1/L2 backlog 再启动 autosync、constraint refresh event、network/timeout/rejection fail-soft 和禁用策略源码边界。
