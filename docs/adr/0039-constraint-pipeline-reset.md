...
**2026-07-08 / 2026-07-21 现状注记（不得把 constraint-shadow 写成当前 session rule authority）**：
- **当前 production session rule authority** 是 ADR 0040 Policy stable-view（`~/.abrain/.state/sediment/proposition-policy-stable-view/v1/latest` 指向的 content-addressed bundle）；live `ruleInjector` 配置为 `propositionPolicyStableViewInjection`，不是 constraint-shadow。
- `~/.abrain/.state/sediment/constraint-shadow/latest/`（含历史 `compiled-view.md` / `decision.json` / `event-coverage.json`）与 git 同步的 `l2/views/constraint/` 保留为 **historical / offline diagnostic / 审计投影 / residual retirement 输入**，**不是**当前 session_start rule authority，也不得被读作“runtime 仍以 constraint-shadow 注入”。
- 2026-07-08 原文记录的 “runtime injection still reads `.state` compiled-view, no read-flip to git L2” 是 **Policy flip 之前** 的中间态描述；在 2026-07-20/21 Policy stable-view runtime flip completed 之后，该句不再描述 live authority。任何 residual compiled-view 物化对齐或 dual-read 退役仍属独立过渡面，不得回写成 live session rule 源。
...
Runtime consumer 默认只读 stable facade / view：`memory_search` 和 `memory_decide` 读 Search Corpus View；**production `session_start` rule authority 读 ADR 0040 Policy stable-view**（不是 constraint-shadow Compiled Constraint View；后者仅 historical/offline/audit residual）；rationale / decision brief 读对应 materialized view。
...
Constraint / Policy freshness 接受 queued / stale。production 注入上一份严格校验的 Policy stable-view（stale 可诊断但仍注入；invalid/missing 为 loud zero），不同步 LLM 裁决；不得回退到 constraint-shadow compiled-view、D3 或 legacy rules 作为 session rule authority。
...
- 2026-07-21：现状注记澄清——constraint-shadow / Compiled Constraint View **不是**当前 session rule authority；production authority 已由 ADR 0040 Policy stable-view runtime flip 接管。本注记只修正过时 authority 表述，不改 ADR 0039 统一 Evidence / L1-L2-L3 决策正文。
