...
| outcome unknown 占比溯因 | 2026-07-08 审计登记 | 当前结论：68.6% 是 missing-used observation bucket，主要由 retrieval-only/tool-result 与 injection-only/path-a-injected 组成；不再当作 classifier/parser unknown 处理，改跟踪 per-source ratio。真正待设计项是 Path A/tool-result usage 信号与 R5 prompt revision proposal。 | per-source ratio 稳定可解释，且 Path A/tool-result usage 信号设计完成或明确保留为 retrieval/injection-only。 | outcome-ledger、2026-07-08 outcome unknown triage、aggregator per-source buckets。 | 继续跟踪 per-source ratio；后续 T0 评审 Path A/tool-result usage 信号与 R5 prompt revision sidecar。 |
