...
| ADR0040 P3 D3-v2 session_start single-consumer | 2026-07-19 | `blocked / separate_authorization_required`；S1/R3.9 execution-ready：Door1复用activation闭集validator、stateRoot===activation.rollback_target、intent parent_hash链、sandbox hard-path覆盖真实control roots、R3.5 path preflight保留；**R3.6关闭OpenAI check-after ancestor-swap窗口**：Linux retained parent-directory FD walk（从/持有父fd，经`/proc/self/fd/<parentFd>/<basename>`做lstat/open/mkdir；下一层fd打开后才释放上一层；任何symlink拒绝；create/write期间目标父fd保持打开）；`ensureDirectoryChainNoSymlink`/`atomicDurableWrite*`/`quarantine rename`均走procfd相对路径（双parent fd）；settings读写锚定；非Linux/proc unavailable fail-closed；Linux boundary文档化；闭集一次性sandbox-only test hook（env token）；RENAME_NOREPLACE dest race残余保留。live selector default-off/absent；dossier NOT_AUTHORIZED/executable=false/candidate null；本轮不调用真实hard-path production rollback。S2/P3未授权。 | 取得绑定activation object与真实/显式candidate session id的独立S2授权；聚合P3仍blocked。 | [execution-ready note R3.9](./notes/2026-07-19-adr0040-d3-v2-session-start-execution-ready.md)、[adapter manifest](./evidence/2026-07-19-adr0040-d3-v2-session-start-adapter-manifest.json)、[dossier](./evidence/2026-07-19-adr0040-d3-v2-session-start-execution-ready-dossier.json)、D3-v2 smokes。 | 禁止启用selector、创建真实session、写production D3/L1/.abrain或对真实hard paths执行生产rollback。 |
| ADR0040 P3 runtime consumer read flips | 2026-07-13 | `blocked / separate_authorization_required`；聚合P3仍blocked。D3-v2 session_start consumer已有S1实现（见独立条目）但仍NOT_AUTHORIZED；Knowledge pull、Policy push、canonical L2等其它runtime consumer均未读取published D3 selection。 | 每个consumer分别完成前置projector、独立授权、rollback与fail-closed定义。 | ADR0040、D3-PUB post-publication dossier、D3-v2 session_start dossier。 | 禁止合并多个consumer flip授权。 |
