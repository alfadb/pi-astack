...
| ADR0040 P3 runtime consumer read flips | 2026-07-13 | `blocked / separate_authorization_required`；D3 generation-0 selection已存在并验证，但当前没有Knowledge pull、Policy push、session_start、canonical L2或其它runtime consumer读取它。 | 每个consumer分别完成前置projector、独立授权、rollback与fail-closed定义。 | ADR0040、D3-PUB post-publication dossier。 | 禁止合并多个consumer flip授权。 |
