...
| ADR0040 P3 D3-v2 session_start single-consumer | 2026-07-19 | `blocked / separate_authorization_required`；R4 production create/bind operator已在immutable R3.9 predecessor上实现并随本release发布：bind-existing-only session path/dev/ino/prefix；fresh execute/continue各自验证trusted persisted transcript exact latest standalone role=user/header/chain/prefix/freshness；`operation_id=SHA256(JCS完整授权tuple)`；intent→一次成型AUTHORIZED activation→settings OFD/raw CAS→exact receipt均为retained-parent-FD + O_EXCL temp + fsync + hardlink no-replace发布；runtime在D3读取前要求settings/intent/activation/receipt/session/operation/source closure全精确，否则selected-zero且无ADR0039 fallback；恢复仅接受冻结矩阵，无自动重试。真实production preview前后snapshot一致且零写；live v2 key仍absent。S2/P3仍未授权。 | 使用已发布clean R4 commit作为`source_commit`绑定后，取得exact fresh trusted-session S2 execute授权；continue和rollback仍各自独立授权；聚合P3保持blocked。 | [R4 design](./notes/2026-07-19-adr0040-d3-v2-session-start-r4-design.md)、[R4 execution-ready note](./notes/2026-07-19-adr0040-d3-v2-session-start-r4-execution-ready.md)、[operator manifest](./evidence/2026-07-19-adr0040-d3-v2-session-start-r4-operator-manifest.json)、[R4 dossier](./evidence/2026-07-19-adr0040-d3-v2-session-start-r4-execution-ready-dossier.json)、[read-only preview](./evidence/2026-07-19-adr0040-d3-v2-session-start-r4-production-read-only-preview.json)、R4 smoke；R3.9 evidence保持原字节。 | 保持S2 NOT_AUTHORIZED；禁止本实现轮启用selector、创建/改写session、写live settings或production intent/activation/receipt/audit目录、调用rollback。 |
| ADR0040 P3 runtime consumer read flips | 2026-07-13 | `blocked / separate_authorization_required`；聚合P3仍blocked。D3-v2 session_start已有R4 create/bind operator与runtime exact gate，但S2仍NOT_AUTHORIZED且live v2 key absent；Knowledge pull、Policy push、canonical L2等其它runtime consumer均未读取published D3 selection。 | 每个consumer分别完成前置projector、独立授权、rollback与fail-closed定义。 | ADR0040、D3-PUB post-publication dossier、D3-v2 R3.9 predecessor与R4 execution-ready dossier。 | 禁止合并多个consumer flip授权。 |
