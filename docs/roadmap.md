...
| 2（下一项） | `RM-LIFECYCLE-002` + `forgetting.kind-evidence-strength-v1` | **staging / pending / E2 / Lane G 自治收敛**：把原 E2 curator 修边、大脑内部 reviewer 与 Lane G G4-G5 合并为一个自治 lifecycle 面。每个状态必须自动进入 terminal disposition，或在带截止/退避/新证据触发条件的可逆非终态（provisional / contested 等）中等待——provisional/contested 不得成为无界非终态。针对 transient / provider failure 自动退避、重试或切换自治 reviewer。Lane G 只保留不违反 P7 gate、且不引入人工 review 的 region/ranking 与收敛能力。 | 用真实 arrival、throughput 证明队列持续收敛：无界 pending=0，oldest-age 受控；不承诺历史条目人为清零。无 review-staging 用户入口、无人工清队列、无永久 `review_required`。大脑内部 reviewer 必须是自治、非人审。`staging.hard-delete` 继续 `blocked / separate_authorization_required`，不属于本任务。 |
