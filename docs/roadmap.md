...
| P4-b retire legacy read fallback | 翻 `compiledViewInjection.fallbackToLegacyOnError=false` 并移除 legacy rules 读兜底。 | **soak-gated**：等 Knowledge projection_only soak 通过后执行（r2 共识 Phase D）；属 flip 类，需 multi-T0 + 用户重启，不在主会话内强翻。 |
