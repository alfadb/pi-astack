...
| 3（blocked / gated） | `RM-FORGET-001` + `forgetting.upstream-wiring` + `forgetting.kind-evidence-strength-v1` | **自治遗忘真实 round-trip**：依赖顺序 1-3。RM-LIFECYCLE-002 的 frontmatter bridge、E2 reconcile、convergence read-model 与 proposal planning/dry-run 保持运行；real demote 同时要求独立、默认 false 且仅字面布尔 true 的 `memory.forgetting.executorRealApplyEnabled` 专用授权，与 effective `sediment.autoLlmWriteEnabled=true` 全局写 authority。后者按既有 schema/resolver 接受布尔 true 和 legacy `"true"`，staging-only/false/missing/malformed 均关闭；任一门不能单独获权。该 hold 对 E1 全 kind（含长尾）及非 E1 一致；archive reactivation 不受影响。 | 当前 production eligible=0 的 gate dossier只证明真实配置与零写，动态 candidate mutation blocking 由 smoke 补充，不算 production demote 验收。解除必须同时具备 RM-FORGET gate dossier、真实 query corpus 的 recall/none pre-demote baseline、闭合 30d observation/reactivation window、long-tail reviewer 与 evidence gates，并取得 fresh independent authorization。之后仍须受控 production 批次、demote ledger、proposal terminal disposition、复活 round-trip 与 corpus 回归共同通过；终点仅 `archived`，禁止 Lane G、人审队列与物理删除。 |
