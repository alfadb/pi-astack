...
| (c)+(d) Batch A 子组 2: stage-index grant isolation E2E | _待推_ | 新 `scripts/smoke-abrain-vault-grant-isolation.mjs` (17 assertion)。Stage 整个 `extensions/abrain/index.ts` (复用 `smoke-abrain-secret-scope.mjs` 的 staging 模式)，通过 `__authorizeVaultReleaseForTests` / `__authorizeVaultBashOutputForTests` 驱动五个 UI substrate 分支。覆盖：**(c)** INV-E grant isolation（会话 grant 不跨 key 泄露 / deny+remember 不跨 key 污染 / PromptDialog substrate 零 module-level state）、**(c)** GPT-5.5 R8 P1#1 fail-closed envelope（`ui.select` 与 `ui.confirm` throw 都返回 `ui_authorization_error` + 正确 ui_path）、**(d)** ui.select fallback path（cachedBuilder=null 走 select；再缺 select 走 confirm；都缺 走 “none”）、**(g)** ui_path 端到端 stamp（overlay/select/confirm/cached/none 五种）。Vault 释放路径 6 + Bash output 路径 5 + grant isolation 3 + telemetry helper 1 + INV-E export shape 1 + cached fast-path 1 = 17 assertion。Negative test 验证：临时把 `overlay` tag 改为 `select` 后 3 条 assertion 立即 fail。**这一路在子组 1 ship 前跑过就能提前抦截 P0** — stage-index smoke 揭示 `authorizeVaultBashOutput` 返回是对象而非字符串，type-refactor mismatch 一发就拓。 |
