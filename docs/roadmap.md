...
| `RM-REL-007` + ADR0040 Tier-1 policy bridge | **自然纠正→policy proposition 真实验收**：2026-07-25 最小闭环实现已落地且生产 `sediment.propositionTier1PolicyWriter.enabled=true` armed；fixture smoke 覆盖幂等/pending marker durable/失败保留/进程重启重放/P2a/force republish/in-flight wave/causal binding/registry drift。不迁移历史 constraint，不恢复 legacy/compiled/D3。**真实验收发现（2026-07-25）**：全局长期用户纠正原文（「以后在所有项目中…除非我明确要求T0做决策」）被 LLM `correction_classifier` 判 `no_signal`，同轮 `directive_recall_audit` `missing=1`，导致无 Constraint Evidence / Tier-1 / proposition bridge。**修复（AI-native）**：主 classifier 改 strong model + prompt v3，显式 future/default directive 为一等语义信号；无 regex/pre-gate 写入授权；scope 由 classifier 结构化 `rule_scope` 贯穿 Constraint/proposition，确定性层不做文本 regex 二判；`directive_recall_audit` 的 recall regex **不参与**新 Tier-1 create 授权（既有 LLM 确认的 contradiction demote 路径边界不属于本次 create 修复）。**仍待下一真实事件验收**，不宣称通过。 | **待重启后新事件**：真实 Tier-1 自然纠正（主 classifier 语义命中）→ constraint evidence durable append → dedicated proposition append → durable pending marker → async force republish → **下一 fresh persisted main session** 读到新 policy item。不得以 fixture 单独宣称闭环已通过；未完成前不删 D3-v2；不得打开 constraint autoRefresh/dual-read 或 D3 fallback。 |
