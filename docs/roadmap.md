...
|---|---|---|
| `RM-REL-006` + [ADR 0044](./adr/0044-central-sediment-edge-authority.md) | **Central sediment（T0 R4）目标 contract**：Pi edge 收缩为 durable capture/witness；中心接 source/job/decision。本仓库 **capture-only protocol shadow**（默认关；`smoke:edge-protocol-shadow` / `dossier:edge-protocol-shadow-production`）。A0 前 local intake 仍唯一 semantic primary；edge shadow **不得**升为第二 primary。[ADR 0045](./adr/0045-sediment-worker-safe-rpc-command.md) 是 Stage A daemon worker 机制，叠在 0044 之上，不是并列 primary。 | **Gate**：真实 Pi JSONL 主链多 turn；≥100 turns × 3 轮；`agent_end`/`agent_settled` p99&lt;100ms；integrity 全过才 `accepted`。权威结果只认 [edge protocol shadow production acceptance](./evidence/2026-07-24-edge-protocol-shadow-production-acceptance.json)。Stage A/B/C 完成前不得宣称 cutover。 |
...
| `RM-GOV-001` + `knowledge.legacy-physical-retirement` + `constraint.dual-read-flip` + `constraint.read-flip-state-to-git-l2` + `constraint.dual-read-audit-retirement` + `constraint.tier2-legacy-write-gate` + `proposition.adr0040-p3-runtime-read-flips` + `proposition.adr0040-p4-legacy-authority-retirement` + `canonical_path.p2` + `canonical_path.p3` + `canonical_path.p4a` + `canonical_path.p4b` | **Transition-register consistency repair**：先修 register 语义，再讨论任何 retirement 或 flip。记录 Knowledge legacy physical retirement 的 `ready_for_decision` 与 ADR0040/canonical P4 `blocked` 冲突；该 `ready_for_decision` 不得启动人的 memory retirement 裁决。Constraint residual exit/consumer 仍引用 pre-Policy authority 的陈旧；将 residual non-Policy consumers、canonical P2 与 canonical P3 拆成独立授权面。`proposition.adr0040-p3-d3-v2-session-start` 已 `completed/not_authorized` retired/never production-authorized（Policy stable-view supersedes；historical evidence 保留），不再作为 active blocked 授权面。 | 这是 docs/governance 一致性修复，不是 memory 条目治理，也不授权任何 mutation。`proposition.adr0040-policy-stable-view-runtime-flip` 已完成且不重开；0040 residual 聚合项不能合成 blanket P3 授权。D3-v2 已 retired；其余冲突项继续按 machine state blocked/gated。 |
...
| `RM-REL-007` + ADR0040 Tier-1 policy bridge | **自然纠正→policy proposition 真实验收（已闭合）**：2026-07-25 最小闭环实现已落地且生产 `sediment.propositionTier1PolicyWriter.enabled=true` armed；fixture smoke 覆盖幂等/pending marker durable/失败保留/进程重启重放/P2a/force republish/in-flight wave/causal binding/registry drift。不迁移历史 constraint，不恢复 legacy/compiled/D3。**先期真实验收发现（2026-07-25）**：全局长期用户纠正曾被 classifier 判 `no_signal`；**修复（AI-native）**：主 classifier strong model + prompt v3，显式 future/default directive 为一等语义信号；scope 走 classifier `rule_scope`；`directive_recall_audit` recall regex 不参与新 Tier-1 create 授权。**生产闭合证据**：user session `019f93cb…` turn `d4559727` → CE `877c79a7…` → Prop `321f9bad…` → bundle `e5c4eff9…` item `bdedeb19…`；held settle 延迟（`CANONICAL_STARTUP_BLOCKED` → 后续 settle）；fresh session `019f9db0…` runtime audit `2026-07-26T09:11:46.340Z` `selected_valid` `item_count=2` view hash `7a7b7347…`。Policy stable-view supersedes；D3-v2 retired/never authorized，historical evidence 保留。 | 生产闭环已证明；fixture 仍非 sole evidence。不得打开 constraint autoRefresh/dual-read 或 D3 fallback；不得恢复 D3-v2 runtime。 |
...
|---|---|---|
| `RM-SEDIMENT-WORKER-001` | **Stage A sediment worker-safe RPC**（[ADR 0045](./adr/0045-sediment-worker-safe-rpc-command.md) 机制，非目标 authority） | daemon 可 spawn `pi --mode rpc` + `PI_ASTACK_SEDIMENT_WORKER_MODE=1` 只加载 sediment extension，用 `/sediment-worker-run` 处理 terminal_witness sidecar；`sediment.executionOwner=daemon` 单执行器；独立 checkpoint slot；success receipt 仅真实 CP 推进+more=false；copy-store path 门禁；复用 `runSedimentAgentEndPass`；**不是** formal ConsumerAck/authority/retention，**不是**并列 primary（叠在 ADR 0044 capture-only 之上；A0 前 local sediment 仍唯一 semantic primary）。后续：pi-router supervisor 接线、真实模型 production bridge、decision/write 计数回传、receipt/claim GC。 |
