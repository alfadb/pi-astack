...
`package.json#scripts` 是 smoke 列表 live truth。当前 **23 个**（2026-05-17 ADR 0022 P1+P2+P3a +5；2026-05-18 R7 OptionList +1），P3b assertion 在 `smoke:abrain-vault-reader` 内 +8（6 → 14）：
...
smoke:abrain-vault-reader              # ADR 0022 P3b: 6 → 14 assertion
...
- **P3b** (2026-05-18)：`authorizeVaultRelease` / `authorizeVaultBashOutput` 主路径迁到 PromptDialog overlay（variant `vault_release` / `bash_output_release`）。新增 `extensions/abrain/vault-authorize.ts` thin wrapper，不走 `service.askPromptUser`（不共享 prompt_user 的 concurrent gate / audit lane / grant 状态，INV-D/E 边界）。`PromptDialog` 扩 `allowOther` flag：variant != "question" 时关闭 Other 选项（vault 决策不允许自由逃逸）；同时隐藏 `(N/M)` progress，提示改为 `enter authorize • esc deny`。`ui.select` 保留为 fallback（`ui.custom` 不可用 / dialog 抛错时）。`smoke:abrain-vault-reader` 6 → 14 assertion。
...
| **E** | `vault-authorize.ts` 模块零 state；dialog substrate 不持 grant。连续 vault_release call 不串话；vault variant 不含 Other | **`smoke:abrain-vault-reader` (P3b +8 assertion)** |
...
- P3b ship 后计划下一轮高价值多-LLM audit（vault state machinery），覆盖：PromptDialog allowOther/variant 分叉、fallback chain 、grant 状态隔离、串行 vault↔prompt_user 不串话、未知 choice defense in depth。
