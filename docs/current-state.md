...
| 证据架构（ADR0039） | 三层 HYBRID_MD_GIT_PLUS_DB：L1 Evidence Event SOT（git）→ L2 Markdown View（git，人类可读审计视图，**非用户编辑面**；用户纠错须转成新 L1 event 再重投影）→ L3 SQLite 派生索引（不入 git、可丢弃重建）。读路径现状：Constraint 注入 compiled-view（live）；Knowledge 读 `projection_with_legacy_fallback`（过渡态，legacy 兜底）。各域迁移阶段/未完成项以 [`docs/roadmap.md`](./roadmap.md) + [ADR0039](./adr/0039-constraint-pipeline-reset.md) 为准。 |
