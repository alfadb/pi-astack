...
| Forgetting 运行实态 | `RM-FORGET-001` 仍为 `blocked / separate_authorization_required`。decay→lifecycle proposal、durable-kind/legacy repair、RM-LIFECYCLE-002 frontmatter bridge、E2 reconcile、convergence refresh 与 proposal planning/dry-run 保持运行；real demote 同时要求字面布尔 `memory.forgetting.executorRealApplyEnabled===true` 与 effective `sediment.autoLlmWriteEnabled=true`。前者提供独立授权且字符串无效；后者按既有 auto-write schema/resolver 接受布尔 true 和 legacy `"true"`，`staging-only`、false/`"false"`、缺失和 malformed 均关闭。任一门不能单独授权。hold 对 E1 全 kind（含长尾）与非 E1 一致，审计 reason 区分 `executor_real_apply_gate_closed` 与 `global_write_authority_gate_closed`；archive reactivation 继续按既有逻辑独立运行。2026-07-23 production dossier 在真实 eligible=0 前提下只证明配置关闭、proposal/durable hash 零变化及 lifecycle read-only hook，不冒充 production candidate blocking 或 demote 验收。解除仍需 gate dossier + recall/none baseline + 闭合 30d window + reviewer/evidence gates + fresh independent authorization；禁止 Lane G、人审队列与 hard-delete。 |
