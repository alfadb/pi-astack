...
| Canonical local convergence | production `canonicalGitRuntime` 当前已显式启用为 `local_convergence_v2`；首笔经单独授权的 exact local drain 已由 [read-only evidence manifest](./evidence/2026-07-12-canonical-path-p1-a-production-existing-local-drain-manifest.json) 验收。该证据只完成 `LOCAL-DRAIN-CURRENT`；后续 writer generation、真实 restart、Curator wiring 与 P1 close 仍未完成，不能宣称 P1 完成。device delivery 继续是 noncanonical best-effort infrastructure。 |
