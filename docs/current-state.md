...
| `extensions/sediment/` | `agent_end` hook；`/sediment status/dedupe`；background lanes | shipped；唯一 dedicated writer；`autoLlmWriteEnabled` default true；prompt-native aggregator v1.1、L1 Evolution Loop v1（`evolution-ledger.ts` internal self-state）、multi-view P0.5、staging-resolver v1 triage、archive-reactivation v1 都已接入。`promote_candidate` 仍是 advisory，不能绕过 multi-view / curator 直接升 durable。 |
