# Current State — pi-astack（2026-05-31）
...
| `extensions/sediment/` | `agent_end` hook；`/sediment status/dedupe`；background lanes | shipped；唯一 dedicated writer；`autoLlmWriteEnabled` default true；prompt-native aggregator v1.2（Step-7 稳定身份 + sidecar identity convergence `adoptUnsluggedAlias`）、L1 Evolution Loop v1（`evolution-ledger.ts` internal self-state）、multi-view P0.5、staging-resolver v1 triage、archive-reactivation v1 都已接入。`promote_candidate` 仍是 advisory，不能绕过 multi-view / curator 直接升 durable。 |
...
`package.json#scripts` 是 smoke 列表 live truth；`docs/reference/smoke-tests.md` 是便于阅读的镜像。当前 **64 个**（2026-05-31），覆盖 memory/sediment、L1 evolution-ledger、dispatch C5/heartbeat、web-search、tool-contract、time/turn-progress、verify-after-edit、edit-strip-empty、CAS/staging/archive-reactivation 等路径。
...
64/64 全绿为 ship 门槛；如本文与 `package.json` 或 `docs/reference/smoke-tests.md` 冲突，以 `package.json` 为准。
