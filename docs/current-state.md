...
| ADR0040 Policy/session-start push | Production full flip is **completed/authorized** (transition `proposition.adr0040-policy-stable-view-runtime-flip`): every persisted main session reads only the strictly validated content-addressed Policy stable view under the current `ABRAIN_ROOT` or `HOME/.abrain`; ephemeral main sessions and subagents are excluded. Stale is diagnostic-only and still injects; invalid/missing is loud zero injection with no compiled/D3/legacy fallback. Per the user's 2026-07-21 full-auto derived-publication authorization, canonical startup/recovery ready schedules a detached deterministic rebuild from canonical whole-L1 -> P2a -> fixed profile -> production publisher; strict compile success may automatically switch `latest` without per-device grant, but recovered is reported only after the strict runtime reader returns `selected_valid`. TUI/RPC session start does not wait; same-root process singleflight and cross-process OFD lock coordinate retries. Reader remains read-only/no lazy repair, and unsafe symlink/foreign/collision residue remains untouched and fail-closed. Production acceptance on 2026-07-21 recorded recovery initial `read_failed` -> status `recovered` -> final `selected_valid` for bundle `028c8d0354f31eae97269d66991d7fedcbd57aad0badbd45e31ca287046f7a2d`; the next real-turn runtime audit recorded `policy_stable_view_injected` / `selected_valid`, item=1, view=341 bytes, fence=1/1, with compiled/D3/legacy markers false. **2026-07-25 Tier-1 bridge**: natural correction → constraint evidence → dedicated policy proposition (`pi-astack.proposition-tier1-policy-writer`) → durable low-sens source-change pending marker (event id only) → async source-change force republish is **implemented and production-armed** (`sediment.propositionTier1PolicyWriter.enabled=true`); capture requires the marker to be durable; successful covering republish deletes the marker only after `selected_valid` under the production runtime read budget (`ruleInjector.propositionPolicyStableViewInjection.maxReadBytes`, not the hard recovery 262144 envelope); session_start canonical-ready rescans markers and force-republishes even when the old stable-view is still strict-valid; envelope remains `defined_inactive` / `write_enabled=false` / `fold=false`; no historical constraint migration; no legacy/compiled/D3 fallback restored. **Acceptance finding (2026-07-25)**: a real global durable user correction was classified `no_signal` by the LLM correction classifier while same-turn `directive_recall_audit` reported `missing=1`, so no Constraint Evidence was written. **Fix (AI-native)**: main correction classifier uses a **strong model + prompt v3** (hot-path cost is an explicit tradeoff for semantic authority; no mechanical cost-reduction gate) — explicit future/default/cross-project USER directives are first-class semantic signals (not only rebuttals of prior memory/behavior); conservative null only for true ambiguity; no regex/pre-gate write authorization. Scope authority is classifier structured `rule_scope` (project|global) throughout Constraint Evidence / Tier-1 / proposition — deterministic layer only enum-checks and binds projectId, with no natural-language regex second-guess of scope. `directive_recall_audit` recall regex does **not** authorize new Tier-1 create; its existing LLM-confirmed CONTRADICT demote path is a separate boundary and is not part of this create-path fix. Deterministic layer stays limited to existing deriveProvenance/schema/sanitize/Tier1 write paths. **Real Tier-1 → fresh session closure (production)**: user session `019f93cb-e736-7110-b6ea-cf3ee07b68be` turn `d4559727` → constraint evidence `877c79a76fc47d…` → dedicated policy proposition `321f9bade0cb9b…` (writer `pi-astack.proposition-tier1-policy-writer`) → stable-view bundle `e5c4eff9f941ff…` item `bdedeb19230b7b…` (project `pi-global`). First authorize was held (`tier1-held`, `constraint_evidence_append_failed:CANONICAL_STARTUP_BLOCKED`); held settled after canonical-ready via authorized held-retry operator `executeTier1HeldAuthorizedRetry` (correlation prefix `tier1-held-operator`); no automatic canonical-ready retry. Fixture/offline smoke remains non-sole evidence. Fresh persisted session `019f9db0-db0b-7661-8b14-6e646e2bcd45` runtime audit `2026-07-26T09:11:46.340Z` recorded `policy_stable_view_injected` / `selected_valid`, `item_count=2`, view hash `7a7b7347540e61…` (no compiled/D3/legacy). **Boundary**: Policy stable-view supersedes D3-v2 as sole session rule authority; D3-v2 is `completed/not_authorized` retired/never production-authorized (runtime deleted; historical evidence retained immutable/non-rerunnable; production D3-PUB v2 read-only). Residual non-Policy P3 consumers and canonical_path subsequent read flips remain `blocked` / separately authorized (see [`transition-register.md`](./transition-register.md)). constraint-shadow compiled-view dual-read/compiled injection path is **retired** (2026-07-27 S0: dualread-audit.ts + compiled/self-heal helpers deleted; residual runtime-gate dossier/smoke + package aliases deleted); residual constraint-shadow compiler remains manual/offline only — not current session rule authority. |
...
| 跨设备同步与 hook 边界 | device delivery 使用用户环境中的 native `git fetch`、确定性 device join 与 exact-OID push。join 对唯一 merge-base 的完整 `B/H/U` trees 执行 L1 add-only union、注册 L2 从 union L1 版本化全量重建、普通 tracked path 文件级三方选择；真实双侧内容冲突 typed fail-closed。发布由 per-repo OFD mutation barrier、完整 `H -> M` journal、CAS、前/后像恢复保护：验证边界是 **delta-exact**（journal path 的 worktree+index 为 exact H 或 M，index 收敛后 readback exact M），**不是** whole-tree clean。delta 外 ordinary staged/untracked/`.gitignore` dirty 原样保留；delta 外 tracked/staged L1 与注册 L2 篡改 fail-closed，untracked valid L1 backlog 可保留，unknown registered L2 仍 fail-closed。清 journal 前验 `HEAD=M`、candidate contract、delta exact M，以及 M 中全部 L1/注册 L2 的 index+worktree 不变量。push rejection 只作有界 fetch/join/push retry。网络/auth/timeout 仍仅作 `.state` audit / warning，不进入 canonical truth 或 local startup gate。pi 不管理 remote、upstream、auth、transport config 或 hooks；`merge-tree`、rebase、force 和 LLM merge 均禁用。`l2/views/knowledge/latest/manifest.json` 是 tracked canonical L2，不再是 device-local ignore。首次切换后所有旧 pi 实例必须重启以加载 OFD barrier。旧 ADR0039 hook installer 的遗留 artifact 仍只通过既有一次性 startup migration 处理。 |
...
- pi 会 await `agent_end`；sediment handler 只持久化轻量 create-only intake receipt（含 canonical `sourceProjectRoot`），绝不等待 canonical startup、Git、classifier、curator 或 projector。Process queue 仅保留同 key latest coalesce/串行、跨 key cap、`more` continuation 与 error containment；旧 readiness/park/wake/TTL/bytes lifecycle 已删除，restart recovery 由 **当前 boot physical project root 拥有的** intake pending 负责（相同 project_id 不同 checkout 隔离；不按全局 projectId 扫描；footer/notify 受 foreground session+epoch fencing）。checkpoint v3 继续提供 lineage + per-candidate idempotency，partial failure 不推进 watermark。ABOUT-ME staging 路径仍 content-addressed；window-bound child work 按 session/resource key 跟踪，不跨 session 等待。
