...
D3 shadow control plane (shadow.ts + shadow smoke/preview) is retired. D3-WF sandbox surface (sandbox-writer/control-v2/v3 + d3-wf smoke/preview that copied production L1) is retired. D3-PUB pre-publication surface (preview/transcript/bootstrap/execute/pre-pub smoke) is also retired; only `smoke:proposition-lifecycle-freshness-d3-pub-post-publication` remains as the published-closure read-only gate. D3-v2 session_start runtime and adapter smokes are retired (modules deleted); historical D3 shadow/D3-v2/D3-WF/D3-PUB evidence is retained immutable/non-rerunnable and is not a current runtime gate. D3 has no production rule-injection call edge or runtime authority. P2a.2.1 (`smoke:proposition-policy-push-publication-p2a21`) and P2a.2.2 (`smoke:proposition-policy-push-live-publication-p2a22`) remain registered for historical pre-publication reproducibility but are intentionally excluded from `smoke:all` and must not be interpreted as current runtime gates.
...
npm run smoke:sediment-extractor-budget
...
`smoke:sediment-intake-publication` covers byte-stable create-only intake and exact Pi JSONL branch restoration, a 5,000-entry capture latency bound without transcript cloning, SIGKILL recovery through outbox/L1/checkpoint/ack, two fresh recovery children with exact-once event identity, fail-closed missing source retention, intake write failure audit/notify without enqueue, unknown outbox domain → `failed/`, detached HEAD → pending retry (not `failed/`), atomic groups larger than the ordinary 64 batch target freezing alone, and exact-window readiness with legacy session fallback. A real cross-process OFD-busy scenario proves two sessions remain intake-pending with zero L1/checkpoint until barrier release, then recover independently and converge L2/Git. Terminal outbox checks prove same-root atomic pending→failed rename, unchanged item bytes, no post-move dual name, identical-destination idempotence, conflicting-destination fail-closed behavior, durable before/after writer audit with closed reason/time/internal identity, audit-I/O failure that logs closed without blocking the transition, `opendir` existence probing, and durable batch cardinality deduplicated by item ID. Its canonical-enabled fixture commits tracked A, leaves valid sibling B untracked with no outbox, freezes batch C, then appends outbox tail D after freeze: first HEAD/L2/manifest contain only A/C, B remains untracked, and D stays pending for the next one-shot. It additionally locks `projectOnWrite=false` L1 inclusion, non-cohort staged preservation, CAS-after-publication crash with pending receipt, HEAD-byte noop replay without a second commit, and no-pending repair-only of dangling L2/manifest with unchanged L1 cardinality. Owner-root isolation asserts two temp git checkouts with the same `project_id` cannot cross-claim pending, legacy v2 receipts without `sourceProjectRoot` still derive owner from cwd, foreground session epoch fencing blocks stale footer updates after `/new`, unbound current-root `path_unconfirmed` still surfaces, and same-root restart recovery continues to ack owned pending. Startup/history recovery and push entry points are not used.

`smoke:lsea-worker-admission` covers the pi-astack worker-first LSEA slice: clean absent-store pairing and intentional fail-closed legacy path anomalies; every strict-record/task/maintenance required-field omission; Unix non-`0600`; read-lock-read byte-restored ABA; stale/unavailable/revoked retryable/no-restart results; real Linux `flock` holder/release; Windows lstat symlink/non-regular rejection and `EBUSY` held versus `EACCES|EPERM` ACL-unavailable classification; foreground store-exists capture-only (held/draining/corrupt/free+held/free+unlocked) versus missing-store legacy posture; rejected task durable zero delta across receipts/checkpoint/L1/outbox/Git HEAD+tree/audit; rejected maintenance progress-before-admission but zero count/drain/repair and durable delta; zero-side-effect capability reporting; and a static assertion that authority admission covers task/maintenance entry plus DCC execution-time mutation frames (re-call `admitLocalExecutorAuthority`) with still no legacy B1-B8 labels. `smoke:sediment-daemon-edge-capture` additionally proves enabled capture-only `session_start` keeps edge candidate-only witness recovery then returns before intake recovery/publication/policy startup, full-triple capture writes candidate+witness, strict foreground capture persists intake without foreground semantic enqueue, `settings.enabled=false` does not reopen authority recovery via the edge triple gate, and `agent_end` fully-disabled returns before authority posture IO.

`smoke:sediment-knowledge-mutations` uses a canonical-enabled real temp Git repository. Fault injection proves outbox failure causes zero L1 writes; with one missing-L1 orphan plus two healthy items, one drain moves both healthy items to done while leaving the orphan pending with `publication_l1_pending`/zero terminal failures; replay fills the same event/item identity without duplicates. It covers update, hard delete, archive, supersede, reactivation-as-update, and multi-event merge accepting at outbox+L1+checkpoint while L2/HEAD remain unchanged. A fresh child then holds the real OFD: accept returns closed `canonical_busy` within the foreground 1s bound and a shorter worker-budget clamp with unchanged L1/outbox/checkpoint bytes; maintenance direct drain returns true `busy` immediately, foreground one-shot preserves `completed`+`lastError`, and release converges through one publisher call. It separately proves scheduler busy returns immediately, incomplete merge batches remain pending, concurrent same-slug events accept from one stable ancestor and fold deterministically, partial merge crash residue completes one stable batch with the same target event, detached HEAD keeps knowledge receipts pending, knowledge outbox stamps `windowId` with exact-window hold, merge members share one window/batch, and replay after stable-parent advancement neither adds L1 nor re-enqueues done outbox work. `smoke:sediment-worker-rpc` covers attempt-local `current_candidate_deferred` (no process sticky session map; recursive candidate cannot advance CP; next terminal isolated), real `_runSedimentAgentEndPassForTests` auto-long/explicit `stagingWritten` trajectories (staging written, CP holds, deferred), attempt-wide partial-CP-then-deferred fatal, `pipeline_threw` / post-pass CP-load invariant, deadline fence preserving `receipt_write_failed` first cause, post-CP audit swallow → processed, same-attempt CP+deferred fatal `deadline_after_checkpoint_advanced`, receipt-preferring cleanup, and `receipt_write_failed` after CP advanced as nonretryable `restart_child`. It also covers the maintenance result closure: held L1 maps to retryable `pending/publication_l1_pending`; timing out only while waiting for global serial maps to retryable `pending/maintenance_worker_busy`, does not poison/restart, and permits the healthy owner plus a subsequent maintenance pass to finish; a drain started by the invocation and left unreaped still maps to `cancel_cleanup_unreaped`, poison, and restart; durable failed residual is sticky critical (`publication_terminal_failed_present`, real `failed_bucket`; symlink/corrupt failed entries fail closed; gate/read failure leaves `failed_bucket=unknown`). `smoke:publication-legacy-repair` uses real temp Knowledge L1/outbox state to prove new world enqueue omits `projectId`, project enqueue preserves exact `project_id`, and explicit single-item legacy repair takes the durable two-stage path normalized pending first, then old failed bytes to resolved, before ordinary drain reaches done. It locks failed `1→0` and resolved `0→1`, extra mismatch/default policy no-op with old result keys, limit `0|1`, both crash cutpoints, two resolved rows with only the second audit missing, audit recovery without L1 or pending/done, retryable closed repair busy/budget, durable closed failure audit/stderr fallback, symlink fail-closed, and result privacy. Resolved count remains test-local and is not an RPC result field. It explicitly does not test or provide ACK/delete/retention/bulk behavior. Status: `accepted` with [production acceptance evidence](../evidence/2026-07-23-sediment-production-acceptance.json). `dossier:sediment-intake-production-readonly` defaults to stdout and writes only with explicit `--output`; it remains read-only and does not consume live intake/outbox.

`smoke:sediment-extractor-budget` locks the extractor semantic-input boundary: a >1.5M-shaped branch with a ~350k `buildRunWindow` must serialize a standalone `bounded_window` prompt that fully includes selected window content and never the pre-window bulk; `branchEntries` / `continuationMessages` cannot inflate prompt chars; over-cap fails closed as `prompt_budget_exceeded` (not provider `llm_error`) with fingerprint status while pending stays unacked; true auth/provider failures stay non-budget; a new fingerprint may retry. `dossier:sediment-extractor-budget-production-readonly` dry-runs the production prompt builder on sessions `019f8914...` / `019f852d...` without LLM calls and writes counts/hashes/IDs only ([evidence](../evidence/2026-07-24-sediment-extractor-budget-production-readonly.json)).
