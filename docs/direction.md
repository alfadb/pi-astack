...
大脑对记忆生命周期**完全自治**（遗忘策略零人类可调参数：忘什么/多快/保护谁/`kind` 权重全由大脑自学自标定）；但任何自治遗忘动作必须**有界可逆**：(a) **无自治物理硬删**——自治遗忘终点是 `archived`（全文留盘、sparse 可达），`git rm` 不在自治授权内；(b) 被降级条目必须有**至少一条运行时可达、不依赖人类 git、也不依赖大脑主动想起**的复活通道（当前：sparse 召回 + 用户纠错）；(c) 将来任何物理回收须先满足 (b) 的检索等价保真度。
