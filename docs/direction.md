...
大脑对记忆生命周期**完全自治**（遗忘策略零人类可调参数：忘什么/多快/保护谁/`kind` 权重全由大脑自学自标定）；但任何自治遗忘动作必须**有界可逆**：(a) **无自治物理硬删**——自治遗忘终点是 `archived`（全文留盘、sparse 可达），`git rm` 不在自治授权内；(b) 被降级条目必须有**至少一条运行时可达、不依赖任何人类手动介入（手动编辑条目 / `git revert`）、也不依赖大脑主动想起**的复活通道（当前：sparse 召回 + 用户纠错）；(c) 将来任何物理回收须先满足 (b) 的检索等价保真度。
**与 INV-GIT-IS-RECOVERY 的边界**：git-as-recovery 对**人类操作的会话内编辑**成立；但自治遗忘的恢复者是**大脑**，而大脑运行时**从不读 git log**，故其复活面必须在工作树内（`archived` 地板）而非 git history。这不是新建专用防护，而是复用既有 `archived` 软删状态 + **拒绝新增**自治销毁能力——与反过度工程一致。
...
- 自治归档误降级（`archived` 终点地板 + 运行时复活通道兜底，见 INV-REVERSIBLE-AUTONOMY；不再单靠 git history）；跨设备最终一致延迟。
...
9. **冷阁（`archived`）按 `kind`/领域分布严重失衡，或 resurrection rate 长期趋零而 corpus 持续膨胀**（自治遗忘策略偏盲、长尾被静默系统性抽空的信号，ADR 0031）→ 回看 ADR 0031 基线，检查衰减是否偏盲。
