...
- **Single executor**: `sediment.executionOwner` default `foreground` (unchanged). When `daemon`, ordinary Pi extension does **not** enqueue / schedule recovery / run normal pass; worker is the sole pass executor. Worker requires `executionOwner=daemon` else `execution_owner_not_daemon`. Full triple gate → continuous edge producer. Incomplete triple gate → **safe fallback** (no silent drop, no dual-write): `edgeProtocolShadow.enabled` → capture-only edge; else `sediment.enabled` → durable local intake **without** enqueue/pass; else aggregate `daemon_capture_disabled` diagnostic.
...
- Success receipt **only** after real checkpoint advance **and** `more=false` (in-worker more loop, budget 16). Soft skip / void ⇒ not processed, no receipt (`retryable=true`). Deterministic skips (`project_not_bound` / `settings_disabled` / `empty_window` / `ephemeral_session`) ⇒ not processed, **`retryable=false`**. Receipt = processed settled success only; no durable failed receipts. Corrupt receipt fail closed. Process-wide pass serial + per-id Linux OFD claim (Stage0 Linux-only; receipts/claims no GC).
- On success, worker drains knowledge publication outbox one-shot under remaining hard budget (or best-effort skip); durable outbox remains if skipped. **All** deadline/unreaped outcomes re-check durable receipt first — receipt present ⇒ settled success, never fatal poison.
- Canonical startup under worker budget ALS clamps busy budget to remaining soft deadline and cooperative-defers; CE/writer share runtime singleton. Worker free-floating auto-refresh/republish/push is suppressed (durable marker/outbox only).
