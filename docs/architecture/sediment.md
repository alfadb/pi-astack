...
Status: **`accepted`** (temp-repo smokes green; production repair-only frozen-publisher evidence accepted 2026-07-23; see `docs/evidence/2026-07-23-sediment-production-acceptance.json`).

Implemented path (code is source of truth):

1. **`agent_end` capture → durable intake** (`extensions/sediment/intake.ts`): create-only fsync under `~/.abrain/.state/sediment/intake/pending/<windowId>.json`. The small record contains immutable Pi session-file/tip coordinates, capture diagnostics, and digests, not a copied transcript. Recovery rebuilds the exact frozen branch with Pi's JSONL parser. `windowId` is sha256 over stable source coordinates (no wall clock). Capture write failure audits + notifies and does **not** enqueue. The lifecycle hook may await this local write only.
2. **Queue driven by intake** (`agent-end-queue.ts` + `index.ts`): the memory queue is only a scheduling optimization; every `session_start` scans pending records for crash/process-restart recovery. It retains only per-key latest coalescing, same-key serialization, cross-key max concurrency, `more` continuation, and error containment. Readiness wait/park/wake/TTL/bytes/ready-pending lifecycle is deleted; durable intake owns restart recovery. Sediment index no longer imports or calls the canonical startup consumer.
