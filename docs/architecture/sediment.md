...
- On settled success / already_processed, worker stamps `publication_pending` from the production outbox metadata-only `hasPending`/count API (empty→false; any durable pending filename→true; read fail→true fail-closed), without deserializing all item content, and does **not** drain publication in-task. Durable outbox + `/sediment-worker-maintenance` (daemon idle owner) own local publication drain — **not** formal ACK / retention / delete. **Deploy worker first**, then daemon that invokes maintenance. **All** deadline/unreaped outcomes re-check durable receipt first — receipt present ⇒ settled success, never fatal poison.
- **Maintenance** (`/sediment-worker-maintenance`, schema `pi-astack/sediment-worker-maintenance/v1`): kind=`publication_outbox` only; `budget_ms` 60s..900s; no project/record/session identity. Requires **effective** owner daemon (full triple gate) plus the normal worker copy-store/non-empty realpath owner allowlist security env; config failures are zero-write. The whole body shares task `withGlobalPassSerial`, so no task pass overlap. Timing out while only waiting for the serial owner means this invocation started no drain: it returns retryable `pending/maintenance_worker_busy`, `restart_child=false`, and does not poison or kill the healthy pass. It directly awaits production `drainKnowledgePublicationOutbox`, which probes the canonical barrier once before batch scheduling; contention returns the real `busy` result immediately without consuming the maintenance budget. Once acquired, frozen-batch cutpoints check AbortSignal/remaining budget. The foreground one-shot keeps its existing `completed` + retryable `lastError` behavior. Real drain result maps: busy→`pending/publication_drain_busy`; held missing L1→`pending/publication_l1_pending`; terminalFailed→nonretryable `failed/publication_terminal_failed` even after=0; other lastError/throw→retryable `failed/publication_drain_failed`; residual→`pending/publication_remaining`; clean empty→`drained`. Status is closed `idle|drained|pending|failed`; pending buckets `unknown|0|1|2-4|5-9|10-49|50+` (unread/not-yet-read is never fake zero); result notify `sediment-worker-maintenance-result:` contains no item id/path/URL/free text. Progress stage `publication` heartbeats every 5s. A drain started by this invocation that remains unreaped after cleanup still returns `cancel_cleanup_unreaped`, poisons, and requests restart; cleanup=0 still observes late rejection.
