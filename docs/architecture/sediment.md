...
- On success, worker stamps `publication_pending=true` and does **not** drain publication in-task; durable outbox + independent maintenance own publication. **All** deadline/unreaped outcomes re-check durable receipt first — receipt present ⇒ settled success, never fatal poison.
