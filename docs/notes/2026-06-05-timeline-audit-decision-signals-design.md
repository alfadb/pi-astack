...
> # ⛔ 状态:CLOSED / 功能已移除(2026-06-05)
> **检索期新鲜度信号(P1 Stage2 freshness + shadow + toggle)已 git revert 移除。**
> 关键洞察(用户提出):第二大脑是**自主演进**的——"谁现在算数"由**写入侧(sediment 取代/更新)**
> 决定并编码进 status/superseded。在正常演进下不会出现"新的错、老的对"的活跃冲突,所以
...
> 不是检索补丁。
>
> **2026-06-05 最终决定:不做。** 纯“按需读简报”(a+c)价值薄:git log + 人脑已覆盖 ~80%,
> 读一次不触发任何行动;真正价值只在信号被**消费**(agent 自动预载 / 大脑自主优先)那一环,
> 而那需要 (b) 自动注入。用户选择不投入。整个“时间维度信号”探索到此结束。
>
> ---
> 以下为已移除功能的历史设计记录,仅供追溯。
