...
### 关联
迁移轨迹（可复现）：`scripts/run-adr-ingest.mjs` + `scripts/verify-rationale.mjs` + `notes/adr0034-ingest/*.json`（17 份分解记录）；分批 abrain commit 链可回滚；原机制 prose git @627de33；living plan 见 `notes/adr0034-impl-plan.md`。能力实现见 `extensions/memory/{direction-impact,ingest-adr,rationale,adr-decomposer}.ts`（123 smoke assertions）。
