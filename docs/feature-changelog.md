...
Knowledge 的 accepted durability 已从 create 扩到 update/delete/archive/supersede/merge/reactivation-as-update：生产 `event_first` + legacy markdown disabled 下先读 stable view 并做 output/watermark/event-identity guard，再解析 replay-stable eventId；同一 canonical mutation barrier 内先 durable create-only 写 publication work，再以该 eventId 幂等追加 L1，只有整组都 durable 才返回 `durable_pending` 并允许 checkpoint 前移。Accept 的 process-singleflight+OFD 总等待上限 1s，worker budget 可更短 clamp；busy 返回 closed `canonical_busy`，不写 outbox/L1、不推进 checkpoint。Outbox 仅引用 `eventId`，不复制 event body 或 L2 payload。Outbox 失败保证 L1 零写；L1 失败返回显式 nonterminal HOLD 且 pending 保留，重试以相同 eventId/outboxId 补齐。merge target/source 的 receipt 不全时保持 nonterminal HOLD。每类 operation 的 `created_at_utc` 来自 immutable source timestamp，缺失以 terminal `source_timestamp_unavailable` fail-closed 并允许 checkpoint 前移，不用 `new Date` fallback，也不重复调用 LLM。`entry_not_found`、duplicate/validation/lint/status-precondition/credential 等确定性 reject 保留原 taxonomy；L1 append write/collision 与 outbox write failure 仍为 nonterminal。

并发 invariant 改为 event topology 权威：L2 stable watermark 可以是当前 L1 的已验证 ancestor；publication lag 不再触发锁内二次 freshness/duplicate 拒绝。并发同 slug event 共享 causal parent，winner 只由 deterministic topo fold 决定。Legacy 配置继续旧同步 markdown/lock/Git 路径。Outbox publisher 在一次 OFD acquisition 后只冻结一次 ready batch（普通目标 <=64，merge 不拆；单原子组 >64 可单独完整 freeze，禁止静默饿死）和 HEAD；每个 Knowledge item 在 selection 前验证其 exact L1 path/registry/canonical bytes/hash/item identity，缺失 L1 作为 held/not-ready 保留同 item/event pending，不进入 batch，也不阻塞其它 ready group；result 以 `publication_l1_pending` + pending>0/terminalFailed=0 闭合。semantic closure 仅为 frozen HEAD tree 中 registry-validated Knowledge L1 + frozen batch validated L1。HEAD 通过 structured `ls-tree` + `cat-file --batch` 读取，projection API 对显式 node set 纯计算，freeze 后新增 L1/outbox 或其他 worktree drift 不进入 cohort。batch 全部 L1、affected L2 output/removal 与同 closure manifest 由一次 temp-index/ref-CAS/index convergence 发布；HEAD dangling watermark/manifest 同批自动修复，无 pending 时显式 one-shot 也可 repair-only。CAS 后 ack 前崩溃由 HEAD byte no-op、index convergence 后再 ack；busy/CAS/index failure 与 detached/unborn/symbolic-ref 暂不可用前不 ack、不进 failed，partial receipt 留 pending；仅 content/schema/identity/path 确定性 terminal 通过同 root atomic rename 进入 `failed/`，identical destination 幂等、冲突 fail closed，durable batch count 按 item ID 去重。Terminal transition 前后追加并 fsync closed writer audit（reason/time/internal item identity）；audit 失败只记 closed log，不阻止 transition，item bytes 不改。Publisher 不调用 startup/requestDrain/whole-history recovery，不新增 push，push 与 ack 解耦。
